...
from celery_worker import bulk_operation_perform_action_functions_haulage,update_multiple_service_objects
...
def create_haulage_freight_rate_bulk_operation(request):
    ...
    params = {'action_name':action_name, 'data':data, 'performed_by_id':request.get('performed_by_id'), 'service_provider_id':request['service_provider_id'],'sourced_by_id':request['sourced_by_id'],'procured_by_id':request['procured_by_id']}
    ...
    bulk_operation_perform_action_functions_haulage.apply_async(kwargs={'action_name':action_name,
    'object':bulk_operation,'sourced_by_id':request["sourced_by_id"],
    'procured_by_id':request['procured_by_id']},queue='low')
