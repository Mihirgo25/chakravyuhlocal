...
def apply_procured_by_id_filter(query, filters):
    return query.join(HaulageFreightRateAudit, on=(HaulageFreightRateAudit.object_id == HaulageFreightRate.id)).where((HaulageFreightRateAudit.object_type == 'HaulageFreightRate') &
