...
def list_haulage_freight_rate(origin_location, destination_location, commodity, container_count, container_type):
    return
...
def get_country_filter(origin_location, destination_location):
    ...
    if locations_data[0]['country_code'] == 'IN' and locations_data[1]['country_code'] == 'IN':
        location_category = 'india'

    if locations_data[0]['country_code'] == 'CN' and locations_data[1]['country_code'] == 'CN':
...
def haulage_rate_calculator(
    ...
    locations_data, location_category = get_country_filter(origin_location, destination_location)

    if location_category not in POSSIBLE_LOCATION_CATEGORY:
        return response

    location_pair_distance = get_distances(origin_location, destination_location, locations_data)
    ...
    container_and_commodity_type = get_container_and_commodity_type(commodity, container_type)

    haulage_freight_rate = list_haulage_freight_rate(origin_location, destination_location, commodity, container_count, container_type)
...
def get_generalized_rates(query, commodity, load_type, container_count, location_pair_distance, container_type):
    final_data = {}
    final_data["distance"] = location_pair_distance
    return


def get_china_rates(query, commodity, load_type, container_count, location_pair_distance, container_type):
    final_data = {}
    final_data["distance"] = location_pair_distance
    return
