...
class HaulageFreightRate(BaseModel):
    ...
    validity_end = DateTimeField(
        default=datetime.datetime.now() + datetime.timedelta(30), null=True
    )
    ...
    def validate_trailer_type(self):
        if (
            self.transport_modes[0] == "trailer"
            and self.trailer_type not in TRAILER_TYPES
        ):
    ...
    def validate_detention_free_time(self):
        if self.detention_free_time is not None:
            if self.transport_modes[0] == "trailer" and self.detention_free_time < 0:
                raise HTTPException(
                    status_code=400, detail="detention free time is invalid"
                )
        else:
            if self.transport_modes[0] == "trailer":
                raise HTTPException(
                    status_code=400, detail="detention free time is required"
                )

    def validate_shipping_line_id(self):
        if not self.shipping_line_id and self.haulage_type == "carrier":
            raise HTTPException(
                status_code=400, detail="shipping line id is required for carrier"
            )
    ...
    def validate_trip_type(self):
        if (
            self.transport_modes[0] == "trailer"
            and self.trip_type
            and self.trip_type not in TRIP_TYPES
        ):
    ...
    def validate_commodity(self):
        if (
            self.container_type
            and self.commodity
            in HAULAGE_CONTAINER_TYPE_COMMODITY_MAPPINGS[f"{self.container_type}"]
        ):
            return True
        else:
    ...
    def validate_invalid_line_items(self):
        ...
        possible_haulage_charge_codes = [
            str(key) for key in self.possible_charge_codes().keys()
        ]
        invalid_haulage_line_items = [
            t for t in haulage_line_item_codes if t not in possible_haulage_charge_codes
        ]
    ...
    def set_platform_price(self):
        ...
        rates = (
            ...
            .where(
                HaulageFreightRate.importer_exporter_id.in_(
                    [None, self.importer_exporter_id]
                )
            )
            .execute()
        )
        ...
        if rates:
            mandatory_line_items = [
                line_item
                for line_item in rates.get("line_items")
                if str((line_item.get("code") or "").upper()) in mandatory_charge_codes
            ]
    ...
    def update_platform_prices_for_other_service_providers(self):
        from services.haulage_freight_rate.haulage_celery_worker import (
            update_haulage_rate_platform_prices_delay,
        )
        ...
        update_haulage_rate_platform_prices_delay.apply_async(
        ...
        for code, line_items in grouped_charge_codes.items():
            ...
            if "slab_cargo_weight_per_container" in code_config["tags"] and flag:
                self.line_items_info_messages[code] = [
                    "can contain slab basis rates for higher conversion"
                ]
    ...
    def possible_charge_codes(self):
        ...
        for code, config in haulage_freight_charges_dict.items():
            if eval(str(config["condition"])) and bool(
                set(self.transport_modes) & set(config["tags"])
            ):
    ...
    def validate_slabs(self):
        ...
        for index, slab in enumerate(slabs):
            if (float(slab["upper_limit"]) <= float(slab["lower_limit"])) or (
                index != 0
                and float(slab["lower_limit"]) <= float(slabs[index - 1]["upper_limit"])
            ):
