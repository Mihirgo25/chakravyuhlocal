...
def get_data(query, filters):
    ...
    for d in data:
        mappings_query = LclFreightRateJobMapping.select(LclFreightRateJobMapping.shipment_service_id,LclFreightRateJobMapping.source_serial_id, LclFreightRateJobMapping.source_id, LclFreightRateJobMapping.shipment_id, LclFreightRateJobMapping.status).where(LclFreightRateJobMapping.job_id == d['id'])
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(LclFreightRateJobMapping.select(LclFreightRateJobMapping.job_id).where(LclFreightRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(LclFreightRateJob.id << job_ids)
