...
def get_data(query, filters):
    ...
    for d in data:
        mappings_query = LclFreightRateJobMapping.select(LclFreightRateJobMapping.shipment_service_id,LclFreightRateJobMapping.shipment_serial_id, LclFreightRateJobMapping.source_id, LclFreightRateJobMapping.shipment_id, LclFreightRateJobMapping.status).where(LclFreightRateJobMapping.job_id == d['id'])
...
def apply_source_filter(query, filters):
    if filters.get('source') and not isinstance(filters.get('source'), list):
        filters['source'] = [filters.get('source')]
    conditions = [LclFreightRateJob.sources.contains(tag) for tag in filters["source"]]
...
def apply_source_id_filter(query, filters):
    ...
    subquery = list(LclFreightRateJobMapping.select(LclFreightRateJobMapping.job_id).where(LclFreightRateJobMapping.source_id << filters['source_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(LclFreightRateJob.id << job_ids)
    return query
...
def apply_shipment_serial_id_filter(query, filters):
    ...
    subquery = list(LclFreightRateJobMapping.select(LclFreightRateJobMapping.job_id).where(LclFreightRateJobMapping.shipment_serial_id << filters['shipment_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(LclFreightRateJob.id << job_ids)
...
def get_reverted_count(mappings_data):
    if mappings_data.shipment_id:
        result = LclFreightRateJobMapping.select(LclFreightRateJobMapping.id).where(
                    (LclFreightRateJobMapping.shipment_id == mappings_data.shipment_id) &
                    (LclFreightRateJobMapping.status == 'reverted')
                ).count()
        return result
    return None
