...
def get_data(query):
    ...
    for d in data:
        mappings_data = LclFreightRateJobMapping.select(LclFreightRateJobMapping.source_id, LclFreightRateJobMapping.shipment_id).where(LclFreightRateJobMapping.job_id == d['id']).first()
        if mappings_data:
