...
def delete_lcl_freight_rate_job(request):
    ...
    if request.get('lcl_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in LclFreightRateJobMapping.select(LclFreightRateJobMapping.job_id).where((LclFreightRateJobMapping.shipment_id == request['shipment_id']) & (LclFreightRateJobMapping.shipment_service_id == request['service_id']))]
...
def update_mapping(status, jobs_ids):
    update_params = {'status': status,  "updated_at": datetime.now()}
    LclFreightRateJobMapping.update(update_params).where(LclFreightRateJobMapping.job_id << jobs_ids, LclFreightRateJobMapping.status.not_in(['completed', 'aborted', 'reverted'])).execute()
