...
def apply_start_date_filter(query, filters):
    ...
    if start_date:
        ...
        query = query.where(
            LclFreightRateJob.updated_at.cast("date") >= start_date.date()
        )
    return query
...
def get_statistics(filters, dynamic_statistics):
    ...
    if (not(filters.get("start_date") or filters.get("end_date"))) and "completed" in filters.get("status"):
        subquery = subquery.where(
            LclFreightRateJob.updated_at.cast("date") == datetime.now().date()
        )
    stats_query = (
...
def build_daily_details(query, statistics, filters):
    ...
    ).where(LclFreightRateJob.status != 'pending').group_by(LclFreightRateJob.status)
...
def apply_date_filter_and_get_pending_count(query, filters):
    ...
    pending_count = query.where(LclFreightRateJob.status == 'pending').count()
