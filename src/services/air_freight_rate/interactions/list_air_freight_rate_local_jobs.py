...
def includes_filters(includes):
    if includes:
        ...
        air_fields = [getattr(AirFreightRateLocalJob, key) for key in required_fcl_fields]
    else:
        air_fields = [
...
def get_reverted_count(mappings_data):
    if mappings_data.shipment_id:
        result = AirFreightRateLocalJobMapping.select(AirFreightRateLocalJobMapping.id).where(
                    (AirFreightRateLocalJobMapping.shipment_id == mappings_data.shipment_id) &
                    (AirFreightRateLocalJobMapping.status == 'reverted')
                ).count()
        return result
    return None
