...
def delete_air_freight_rate_local_job(request):
    ...
    if request.get('air_freight_rate_local_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in AirFreightRateLocalJobMapping.select(AirFreightRateLocalJobMapping.job_id).where((AirFreightRateLocalJobMapping.shipment_id == request['shipment_id']) & (AirFreightRateLocalJobMapping.shipment_service_id == request['service_id']))]
