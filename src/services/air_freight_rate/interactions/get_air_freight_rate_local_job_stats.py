...
def get_air_freight_rate_local_job_stats(filters={}):
    query = get_query()
...
def build_weekly_details(query, statistics):
    ...
    weekly_stats_query = query.select(
        ...
        AirFreightRateLocalJob.status, AirFreightRateLocalJob.created_at.cast("date")
    )
