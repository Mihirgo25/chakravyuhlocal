...
def get_air_freight_rate_local_job_stats(
    filters={}
):
    query = get_query()
...
def get_query():
    query = AirFreightRateLocalJob.select(AirFreightRateLocalJob.id)
    return query
...
def apply_start_date_filter(query, filters):
    ...
    if start_date:
        ...
        query = query.where(AirFreightRateLocalJob.updated_at.cast("date") >= start_date.date())
    return query
...
def apply_source_id_filter(query, filters):
    ...
    subquery = list(AirFreightRateLocalJobMapping.select(AirFreightRateLocalJobMapping.job_id).where(AirFreightRateLocalJobMapping.source_id << filters['source_id']).dicts())
    job_ids = []
...
def apply_end_date_filter(query, filters):
    ...
    if end_date:
        ...
        query = query.where(AirFreightRateLocalJob.updated_at.cast("date") <= end_date.date())
    return query


def get_statistics(filters, dynamic_statistics):
    subquery = AirFreightRateLocalJob.select(
        fn.UNNEST(AirFreightRateLocalJob.sources).alias("element")
    ).alias("elements")
    ...
    if (not(filters.get("start_date") or filters.get("end_date"))) and "completed" in filters.get("status"):
        subquery = subquery.where(
            AirFreightRateLocalJob.updated_at.cast("date") == datetime.now().date()
        )
    stats_query = (
...
def build_daily_details(query, statistics, filters):
    ...
    query = query.select(
        AirFreightRateLocalJob.status, fn.COUNT(AirFreightRateLocalJob.id).alias("count")
    ).where(AirFreightRateLocalJob.status.not_in(['skipped', 'pending'])).group_by(AirFreightRateLocalJob.status)
...
def apply_date_filter_and_get_pending_count(query, filters):
    ...
    pending_count = query.where(AirFreightRateLocalJob.status == 'pending').count()
...
def build_weekly_details(query, statistics, filters):
    query = query.where(
        AirFreightRateLocalJob.created_at.cast("date")
        >= datetime.now().date() - timedelta(days=6)
    )

    query = apply_source_filter(query, filters)
    weekly_stats_query = query.select(
        AirFreightRateLocalJob.status,
        fn.COUNT(AirFreightRateLocalJob.id).alias("count"),
        AirFreightRateLocalJob.created_at.cast("date").alias("created_at"),
    ).group_by(AirFreightRateLocalJob.status, AirFreightRateLocalJob.created_at.cast("date"))
...
def get_all_backlogs(filters):
    query = AirFreightRateLocalJob.select()
    if filters.get('user_id'):
        query = query.where(AirFreightRateLocalJob.user_id == filters.get('user_id'))
    backlog_count = query.where(AirFreightRateLocalJob.status == 'backlog').count()

    return backlog_count

def apply_is_flash_booking_reverted_filter(query, filters):
    if filters.get('is_flash_booking_reverted'):
        query = query.join(AirFreightRateLocalJobMapping, on=(AirFreightRateLocalJobMapping.job_id == AirFreightRateLocalJob.id)).where(AirFreightRateLocalJobMapping.status == 'reverted')
    else:
        query = query.join(AirFreightRateLocalJobMapping, on=(AirFreightRateLocalJobMapping.job_id == AirFreightRateLocalJob.id)).where(AirFreightRateLocalJobMapping.status != 'reverted')
    return query
