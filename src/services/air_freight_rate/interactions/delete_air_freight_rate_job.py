...
def delete_air_freight_rate_job(request):
    ...
    if request.get('air_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in AirFreightRateJobMapping.select(AirFreightRateJobMapping.job_id).where(AirFreightRateJobMapping.shipment_id == request['shipment_id'])]
