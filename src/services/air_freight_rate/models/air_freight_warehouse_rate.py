...
class AirFreightWarehouseRates(BaseModel):
    ...
    def validate_airport_id(self):
        ...
        if airport:
            ...
            self.location_ids = [
                uuid.UUID(str(x))
                for x in [self.airport_id, self.country_id, self.trade_id]
                if x is not None
            ]
        else:
    ...
    def update_freight_object(self):
        ...
        freight_query = (
            AirFreightRate.update(warehouse_rate_id=self.id)
            .where(
                AirFreightRate.commodity == self.commodity,
                AirFreightRate.service_provider_id == self.service_provider_id,
            )
            .where(AirFreightRate.warehouse_rate_id.is_null())
            .where(
                getattr(AirFreightRate, f"{location_key}_airport_id") == self.airport_id
            )
            .execute()
        )
    ...
    def possible_charge_codes(self):
        ...
        air_freight_local_charges_dict = AIR_FREIGHT_WAREHOUSE_CHARGES.get()
        charge_codes = {}
        for code, config in air_freight_local_charges_dict.items():
            if (
                ...
                and "deleted" not in config["tags"]
            ):
    ...
    def update_line_item_messages(self):
        ...
        for code, line_items in grouped_charge_codes.items():
            ...
            if line_items["unit"] not in code_config["units"]:
                line_items_error_messages[code] = [
            ...
            if (
                "tags" in code_config
                and "weight_slabs_required" in code_config["tags"]
                and any(not t.get("slabs") for t in line_items)
            ):
                self.line_items_error_messages[line_items["code"]] = [
                    "weight slabs are required"
                ]
        ...
        for code, config in possible_charge_codes.items():
            if (
                "additional_service" in config["tags"]
                or "shipment_execution_service" in config["tags"]
            ):
                if grouped_charge_codes.get(
                    code
                ) is None and not line_items_error_messages.get(code):
                    line_items_info_messages[code] = [
