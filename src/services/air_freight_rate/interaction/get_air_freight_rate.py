...
def get_air_freight_rate(request):
    ...
    if request.get('weight') and request.get('volume') and request.get('cargo_readiness_date'):
...
def build_freight_object(freight_validity,required_weight,request):
    ...
    validity_start=datetime.strptime(freight_validity['validity_start'],'%Y-%m-%dT%H:%M:%S.%fz').date()
    validity_end=datetime.strptime(freight_validity['validity_end'],'%Y-%m-%dT%H:%M:%S.%fz').date()

    if validity_start >request.get('validity_end').date() or validity_end<=request.get('validity_start').date() or  request.get('cargo_readiness_date').date() < validity_start or request.get('cargo_readiness_date').date() > validity_end:
        return None
...
def get_density_wise_rate(density_params):
    ratio=density_params['gross_volume'] * AIR_STANDARD_VOLUMETRIC_WEIGHT_CONVERSION_RATIO / density_params['gross_weight']
    ...
    elif ratio <= high_density_upper_limit and density_params['chargeable_weight'] >= 100:
        density_category = 'high_density'

    density_category ='general'

    if density_category == density_params['freight_object']['density_category']:
        if density_category == 'general' or (density_category == 'low_density' and int(density_params['freight_object']['min_density_weight']) == int(density_weight)):
            return density_params['freight_object']
        elif density_category=='high_density' and density_params['freight_object']['min_density_weight'] <= density_weight and density_params['freight_object']['max_density_weight'] > density_weight:
            return density_params['freight_object']
        else:
            return  None
    else:
...
def get_freight_object(freight_validity,request):
    ...
    start=datetime.strptime(freight_validity['validity_start'],'%Y-%m-%dT%H:%M:%S.%fz').date()
    end=datetime.strptime(freight_validity['validity_end'],'%Y-%m-%dT%H:%M:%S.%fz').date()
...
def get_required_weight_slab(freight_validity, required_weight,request):
    ...
    for weight_slab in all_weight_slabs:
        if weight_slab['lower_limit']<=required_weight and weight_slab['upper_limit']>required_weight:
            return weight_slab
...
def find_object(request):
  ...
  object = AirFreightRate.select().where(
