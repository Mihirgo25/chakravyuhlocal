...
def update_air_freight_rate_local_jobs_to_backlog():
    ...
    while True:
        air_local_conditions = (
            AirFreightRateLocalJob.created_at
        ...
        affected_ids = jsonable_encoder(
            [
                job.id
                for job in AirFreightRateLocalJob.select(AirFreightRateLocalJob.id)
                .where(*air_local_conditions)
                .limit(BATCH_SIZE)
            ]
        )
        ...
        air_query = AirFreightRateLocalJob.update(status="backlog").where(
            AirFreightRateLocalJob.id.in_(affected_ids)
        )

        rows_updated = air_query.execute()

        AirFreightRateLocalJobMapping.update(status="backlog").where(
            AirFreightRateLocalJobMapping.job_id.in_(affected_ids)
        ).execute()
