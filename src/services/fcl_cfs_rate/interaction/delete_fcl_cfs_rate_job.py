...
def delete_fcl_cfs_rate_job(request):
    ...
    if request.get('fcl_cfs_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in FclCfsRateJobMapping.select(FclCfsRateJobMapping.job_id).where((FclCfsRateJobMapping.shipment_id == request['shipment_id']) & (FclCfsRateJobMapping.shipment_service_id == request['service_id']))]
