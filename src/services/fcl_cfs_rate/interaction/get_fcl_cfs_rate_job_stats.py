...
def get_fcl_cfs_rate_job_stats(
    filters={}
):
    query = get_query()
...
def get_query():
    query = FclCfsRateJob.select(FclCfsRateJob.id)
    return query
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(FclCfsRateJobMapping.select(FclCfsRateJobMapping.job_id).where(FclCfsRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(FclCfsRateJob.id << job_ids)
...
def get_statistics(filters, dynamic_statistics):
    subquery = FclCfsRateJob.select(
        fn.UNNEST(FclCfsRateJob.sources).alias("element")
    ).alias("elements")
...
def build_weekly_details(query, statistics, filters):
    ...
    weekly_stats_query = query.select(
        ...
        FclCfsRateJob.created_at.cast("date").alias("created_at"),
    ).group_by(FclCfsRateJob.status, FclCfsRateJob.created_at.cast("date"))
...
def get_all_backlogs(filters):
    query = FclCfsRateJob.select()
    if filters.get('user_id'):
        query = query.where(FclCfsRateJob.user_id == filters.get('user_id'))
    backlog_count = query.where(FclCfsRateJob.status == 'backlog').count()
