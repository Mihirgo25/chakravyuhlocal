...
possible_direct_filters = ['feedback_type', 'performed_by_org_id', 'performed_by_id', 'status', 'closed_by_id', 'country_id', 'trade_type', 'port_id', 'trade_id', 'service_provider_id', 'spot_search_serial_id', 'container_size','container_type','commodity']
...
def list_fcl_cfs_rate_feedbacks(filters = {}, spot_search_details_required=False, page_limit =10, page=1, performed_by_id=None, is_stats_required=True):
...
def get_page(query, page, page_limit):
    query = query.order_by(FclCfsRateFeedback.created_at.desc(nulls='LAST')).paginate(page, page_limit)
    return query
...
def apply_relevant_supply_agent_filter(query, filters):
    ...
    query = query.where((FclCfsRateFeedback.port_id << port_id) |
                    (FclCfsRateFeedback.country_id << port_id))
    return query
...
def apply_validity_start_greater_than_filter(query, filters):
    query = query.where(FclCfsRateFeedback.created_at.cast('date') >= datetime.fromisoformat(filters['validity_start_greater_than'].split('T')[0]).date())
    return query

def apply_validity_end_less_than_filter(query, filters):
    query = query.where(FclCfsRateFeedback.created_at.cast('date') <= datetime.fromisoformat(filters['validity_end_less_than'].split('T')[0]).date())
    return query

def apply_similar_id_filter(query, filters):
    feedback_data = FclCfsRateFeedback.select(
            FclCfsRateFeedback.port_id, 
            FclCfsRateFeedback.commodity, 
            FclCfsRateFeedback.trade_type
        ).where(
            FclCfsRateFeedback.id==filters['similar_id']
        ).first()
