...
class FclCfsRateJobMapping(BaseModel):
    ...
    job_id = ForeignKeyField(FclCfsRateJob,to_field="id")
    created_at = DateField(default=datetime.datetime.now)
    updated_at = DateField(default=datetime.datetime.now, index = True)

    class Meta:
            table_name = 'fcl_cfs_rate_job_mappings'

    def save(self, *args, **kwargs):
        self.updated_at = datetime.datetime.now()
        return super(FclCfsRateJobMapping, self).save(*args, **kwargs)
