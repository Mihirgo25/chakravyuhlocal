...
from configs.fcl_cfs_rate_constants import (
...
class FclCfsRate(BaseModel):
    ...
    def validate_duplicate_line_items(self):
        ...
        for cfs_line_item in self.line_items:
            unique_items.add(
                str(cfs_line_item["code"]).upper()
                + str(cfs_line_item.get("location_id"))
            )
    ...
    def validate_invalid_line_items(self):
        ...
        invalid_customs_line_items = [
            t for t in cfs_line_item_codes if t not in possible_cfs_charge_codes
        ]
    ...
    def set_location(self):
        ...
        if location["list"]:
            ...
            self.location_type = (
                ...
                else self.location_type
            )
            self.country_id = (
                uuid.UUID(location_data.get("country_id"))
                if self.country_id is None
                else self.country_id
            )
            self.trade_id = (
                uuid.UUID(location_data.get("trade_id"))
                if self.trade_id is None
                else self.trade_id
            )
            self.continent_id = (
                uuid.UUID(location_data.get("continent_id"))
                if self.continent_id is None
                else self.continent_id
            )
            self.location_ids = [
                self.location_id,
                self.country_id,
    ...
    def mandatory_cfs_charge_codes(self):
        return [
            code.upper()
            for code, config in (self.possible_cfs_charge_codes() or {}).items()
    ...
    def set_platform_price(self):
        ...
        rates_query = FclCfsRate.select(FclCfsRate.line_items).where(
        ...
        for rate in rates:
            ...
            for line_item in rate.get("line_items"):
                rate_min_price += common.get_money_exchange_for_fcl(
                    {
                        "price": line_item.get("price"),
                        "from_currency": line_item.get("currency"),
                        "to_currency": currency,
    ...
    def update_platform_prices_for_other_service_providers(self):
        ...
        update_fcl_cfs_rate_platform_prices_delay.apply_async(
    ...
    def update_line_item_messages(self):
        ...
        for code, line_items in grouped_charge_codes.items():
            ...
            if (
                len(
                    set(map(lambda x: x["unit"], line_items))
        ...
        for code, config in filter(
            lambda x: "mandatory" in x[1]["tags"], possible_charge_codes_values.items()
        ):
            code = str(code)
        ...
        for code, config in filter(
            lambda x: "additional_service" in x[1]["tags"]
            or "shipment_execution_service" in x[1]["tags"],
            possible_charge_codes_values.items(),
        ):
            code = str(code)
            if not grouped_charge_codes.get(code):
