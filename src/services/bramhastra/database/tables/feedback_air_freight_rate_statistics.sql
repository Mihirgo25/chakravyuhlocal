...
CREATE TABLE brahmastra.kafka_feedback_air_freight_rate_statistics
(
    `data` String
)
ENGINE = Kafka('127.0.0.1:29092', 'arc.public.feedback_air_freight_rate_statistics', '001','JSONAsString');

CREATE MATERIALIZED VIEW brahmastra.air_freight_before_feedback_rate_statistics TO brahmastra.feedback_air_freight_rate_statistics
(
    `id` UInt256,
...
SELECT
    ...
    JSONExtractUInt(data, 'before', 'air_freight_rate_statistic_id') AS air_freight_rate_statistic_id,
    ...
    FROM brahmastra.kafka_feedback_air_freight_rate_statistics
    WHERE JSONExtract(data,'op','String') = 'u';

CREATE MATERIALIZED VIEW brahmastra.air_freight_after_feedback_rate_statistics TO brahmastra.feedback_air_freight_rate_statistics
(
    `id` UInt256,
    `air_freight_rate_statistic_id` UInt256,
    `feedback_id` UUID,
    ...
    JSONExtractUInt(data, 'after', 'air_freight_rate_statistic_id') AS air_freight_rate_statistic_id,
