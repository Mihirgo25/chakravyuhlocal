...
def get_fcl_freight_map_view_statistics(filters):
    ...
    queries = [
        f'SELECT {",".join(grouping)},AVG(rate_deviation_from_booking_rate) as deviation_from_booking_rate FROM brahmastra.fcl_freight_rate_statistics'
    ]
...
def get_add_group_and_order_by(queries,grouping):
    ...
    queries.append('ORDER BY deviation_from_booking_rate')

def alter_filters_for_map_view(filters,grouping):
    if "origin" in filters:
        index_to_look = HEIRARCHY.index(filters["origin"]["type"]) - 1
        if index_to_look and "destination" in filters and filters["destination"]["type"] == filters["origin"]["type"]:
            ...
            destination_key = f"destination_{filters['origin']['type']}_id"
        filters[origin_key] = filters["origin"]['id']
