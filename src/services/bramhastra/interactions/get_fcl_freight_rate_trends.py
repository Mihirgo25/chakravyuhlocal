...
from services.bramhastra.enums import FclParentMode
...
def get_rate(filters, where):
    ...
    queries = [
        """SELECT parent_mode as mode,toDate(day) AS day,AVG(abs(standard_price)*sign) AS average_price FROM (SELECT arrayJoin(range(toUInt32(validity_start), toUInt32(validity_end) - 1)) AS day,standard_price,parent_mode,sign FROM brahmastra.fcl_freight_rate_statistics"""
    ]
...
def format_response(response, needed_modes):
    ...
    for entry in response:
        ...
        response_dict[day].update({entry["mode"]: entry["average_price"]})
