...
from services.bramhastra.enums import RedisKeys, AirParentMode, AirFilterTypes
...
MAX_ALLOWABLE_DEFAULT_DIFF = 100
...
def get_air_freight_rate_charts(filters):
    where = get_direct_indirect_filters(filters, date=AirFilterTypes.time_series.value)
    ...
    match chart_type:
        case "deviation":
            resp["deviation"] = get_air_freight_rate_differences(filters)
...
def get_accuracy(filters, where):
    ...
    queries = [
        f"""SELECT parent_mode as mode,toDate(updated_at) AS day,(SUM(standard_price_accuracy*sign)/COUNT(DISTINCT id)) AS average_accuracy FROM brahmastra.{AirFreightAction._meta.table_name}"""
    ]

    if where:
        queries.append(" WHERE ")
        queries.append(where)
        queries.append(f"AND standard_price_accuracy != 0")
...
def get_spot_search_to_checkout_count(filters, where):
    ...
    queries = [
        f"""SELECT FLOOR((1 - SUM(checkout*sign)/COUNT(DISTINCT spot_search_id)),2)*100 as spot_search_to_checkout_count from brahmastra.{AirFreightAction._meta.table_name}"""
    ]
...
def get_rate_count_with_deviation_more_than_x_price(filters, where):
    ...
    queries = [
        f"""SELECT count(DISTINCT rate_id) as rate_count_with_deviation_more_than_x_price from brahmastra.{AirFreightAction._meta.table_name} WHERE ABS(standard_price_diff_from_selected_rate) >= {MAX_ALLOWABLE_DEFAULT_DIFF}"""
    ]
...
def format_charts(charts, mode=None):
    if mode:
        ...
        NEEDED_MODES = [i.value for i in AirParentMode]
