...
def get_air_freight_rate_distribution(filters):
    ...
    queries = [
        f"""WITH rate_distribution as 
               ...
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.cancelled.name}' THEN 1 END) AS shipment_cancelled_count,
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.completed.name}' THEN 1 END) AS shipment_completed_count,
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.confirmed_by_importer_exporter.name}' THEN 1 END) AS shipment_confirmed_by_importer_exporter_count,
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.aborted.name}' THEN 1 END) AS shipment_aborted_count,
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.shipment_received.name}' THEN 1 END) AS bookings_created,
               COUNT(CASE WHEN {AirFreightAction.shipment_state.name} = '{ShipmentState.in_progress.name}' THEN 1 END) AS shipment_in_progress_count
    ...
    queries.append(
        f"""
            ...
            FROM brahmastra.{AirFreightRateStatistic._meta.table_name} GROUP BY parent_mode
        )
            SELECT 
...
def get_total_rate_count(filters, where):
    queries = [
        f"SELECT COUNT(DISTINCT rate_id) as count FROM brahmastra.{AirFreightRateStatistic._meta.table_name}"
    ]
    if where:
        queries.append("WHERE")
        queries.append(where)
