...
class GG:
    def get_fcl_freight_rate_lifecycle(filters):
        ...
        print(missing_rates_statistics)
...
def get_missing_rates(filters, where):
    ...
    # query1 = ["""
    #     WITH
    #     missing_rates AS (SELECT count(id) AS count FROM brahmastra.fcl_freight_rate_request_statistics
    #     ),
    #     rate_reverted AS (SELECT count(id) AS count FROM brahmastra.fcl_freight_rate_request_statistics
    #     ),
    #     spot_search AS (SELECT count(id) AS count FROM brahmastra.fcl_freight_rate_statistics
    #     )
    ...
    query4 = ["""SELECT * FROM missing_rates, rate_reverted, spot_search"""]
    # if where:
    #     where_clause = "WHERE " + where + ")"
    #     # query1.append("WHERE ")
    #     query1.append(where_clause)
    #     # query2.append("WHERE ")
    #     query2.append(where_clause)
    #     # query3.append("WHERE ")
    #     query3.append(where_clause)
    # full_query = ' '.join(query1) + ' '.join(query2)+' '.join(query3)+' '.join(query4)
#     full_query=["""WITH
#     missing_rates AS
#     (
#         SELECT count(id) AS count
#         FROM brahmastra.fcl_freight_rate_request_statistics
#     ),
#     rate_reverted AS
#     (
#         SELECT count(id) AS count
...
#     )
# SELECT *
# FROM missing_rates, rate_reverted, spot_search
...
# FROM missing_rates, rate_reverted, spot_search"""]
    print(full_query)   
    full_query = jsonable_encoder(clickhouse.execute(full_query))
