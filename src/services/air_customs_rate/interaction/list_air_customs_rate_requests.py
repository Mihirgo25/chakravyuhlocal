...
def apply_validity_start_greater_than_filter(query, filters):
    return query.where(AirCustomsRateRequest.created_at.cast('date') >= datetime.fromisoformat(filters['validity_start_greater_than']).date())

def apply_validity_end_less_than_filter(query, filters):
    return query.where(AirCustomsRateRequest.created_at.cast('date') <= datetime.fromisoformat(filters['validity_end_less_than']).date())
