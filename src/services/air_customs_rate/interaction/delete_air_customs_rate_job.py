...
def delete_air_customs_rate_job(request):
    ...
    if request.get('air_customs_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in AirCustomsRateJobMapping.select(AirCustomsRateJobMapping.job_id).where((AirCustomsRateJobMapping.shipment_id == request['shipment_id']) & (AirCustomsRateJobMapping.shipment_service_id == request['service_id']))]
