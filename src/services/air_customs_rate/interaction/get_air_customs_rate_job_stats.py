...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(AirCustomsRateJobMapping.select(AirCustomsRateJobMapping.job_id).where(AirCustomsRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(AirCustomsRateJob.id << job_ids)
