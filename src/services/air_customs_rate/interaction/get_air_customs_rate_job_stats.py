...
def get_query():
    query = AirCustomsRateJob.select(AirCustomsRateJob.id)
    return query
...
def apply_start_date_filter(query, filters):
    if filters.get("start_date"):
        ...
        query = query.where(AirCustomsRateJob.updated_at.cast("date") >= start_date.date())
    return query
...
def apply_source_id_filter(query, filters):
    ...
    subquery = list(AirCustomsRateJobMapping.select(AirCustomsRateJobMapping.job_id).where(AirCustomsRateJobMapping.source_id << filters['source_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(AirCustomsRateJob.id << job_ids)
    return query
...
def apply_shipment_serial_id_filter(query, filters):
    ...
    subquery = list(AirCustomsRateJobMapping.select(AirCustomsRateJobMapping.job_id).where(AirCustomsRateJobMapping.shipment_serial_id << filters['shipment_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(AirCustomsRateJob.id << job_ids)
...
def apply_end_date_filter(query, filters):
    if filters.get("end_date"):
        ...
        query = query.where(AirCustomsRateJob.updated_at.cast("date") <= end_date.date())
    return query


def get_statistics(filters, dynamic_statistics):
    subquery = AirCustomsRateJob.select(
        fn.UNNEST(AirCustomsRateJob.sources).alias("element")
    ).alias("elements")
    ...
    if (not(filters.get("start_date") or filters.get("end_date"))) and "completed" in filters.get("status"):
        subquery = subquery.where(
            AirCustomsRateJob.updated_at.cast("date") == datetime.now().date()
        )
    stats_query = (
        AirCustomsRateJob.select(
...
def build_daily_details(query, statistics, filters):
    ...
    query = query.select(
        AirCustomsRateJob.status, fn.COUNT(AirCustomsRateJob.id).alias("count")
    ).where(AirCustomsRateJob.status != 'pending').group_by(AirCustomsRateJob.status)
...
def apply_date_filter_and_get_pending_count(query, filters):
    ...
    pending_count = query.where(AirCustomsRateJob.status == 'pending').count()
...
def build_weekly_details(query, statistics, filters):
    ...
    weekly_stats_query = query.select(
        ...
        AirCustomsRateJob.created_at.cast("date").alias("created_at"),
    ).group_by(AirCustomsRateJob.status, AirCustomsRateJob.created_at.cast("date"))
...
def apply_is_flash_booking_reverted_filter(query, filters):
    if filters.get('is_flash_booking_reverted'):
        query = query.join(AirCustomsRateJobMapping, on=(AirCustomsRateJobMapping.job_id == AirCustomsRateJob.id)).where(AirCustomsRateJobMapping.status == 'reverted')
    else:
        query = query.join(AirCustomsRateJobMapping, on=(AirCustomsRateJobMapping.job_id == AirCustomsRateJob.id)).where(AirCustomsRateJobMapping.status != 'reverted')
    return query
