...
from services.supply_tool.supply_tool_params import DeleteFclFreightRateJob, DeleteAirFreightRateJob
...
def list_air_freight_rate_coverages_api(
    ...
    try:
        data = list_air_freight_rate_coverages(filters, page_limit, page, sort_by, sort_type, generate_csv_url)
        return JSONResponse(status_code=200, content=json_encoder(data))
    except HTTPException as e:
        raise
    except Exception as e:
