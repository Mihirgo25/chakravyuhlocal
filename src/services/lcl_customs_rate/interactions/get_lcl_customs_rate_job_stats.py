...
def get_lcl_customs_rate_job_stats(filters={}):
    query = get_query()
...
def get_query():
    query = LclCustomsRateJob.select(LclCustomsRateJob.id)
    return query
...
def get_statistics(filters, dynamic_statistics):
    subquery = LclCustomsRateJob.select(
        fn.UNNEST(LclCustomsRateJob.sources).alias("element")
    ).alias("elements")
...
def build_weekly_details(query, statistics, filters):
    ...
    weekly_stats_query = query.select(
        ...
        LclCustomsRateJob.created_at.cast("date").alias("created_at"),
    ).group_by(LclCustomsRateJob.status, LclCustomsRateJob.created_at.cast("date"))
...
def get_all_backlogs(filters):
    query = LclCustomsRateJob.select()
    if filters.get('user_id'):
        query = query.where(LclCustomsRateJob.user_id == filters.get('user_id'))
