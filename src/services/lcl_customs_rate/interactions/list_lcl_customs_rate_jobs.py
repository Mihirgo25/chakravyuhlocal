...
def get_data(query, filters):
    ...
    for d in data:
        mappings_query = LclCustomsRateJobMapping.select(LclCustomsRateJobMapping.shipment_service_id, LclCustomsRateJobMapping.source_serial_id, LclCustomsRateJobMapping.source_id, LclCustomsRateJobMapping.shipment_id, LclCustomsRateJobMapping.status).where(LclCustomsRateJobMapping.job_id == d['id'])
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(LclCustomsRateJobMapping.select(LclCustomsRateJobMapping.job_id).where(LclCustomsRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(LclCustomsRateJob.id << job_ids)
...
def get_reverted_count(mappings_data):
    if mappings_data.shipment_id:
        result = LclCustomsRateJobMapping.select(LclCustomsRateJobMapping.id).where(
                    (LclCustomsRateJobMapping.shipment_id == mappings_data.shipment_id) &
                    (LclCustomsRateJobMapping.status == 'reverted')
                ).count()
        return result
    return None
