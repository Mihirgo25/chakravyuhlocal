...
from services.fcl_freight_rate.models.fcl_freight_rate_local_data import (
    FclFreightRateLocalData,
)
from services.fcl_freight_rate.models.fcl_freight_rate_free_day import (
    FclFreightRateFreeDay,
)
from configs.global_constants import (
    DEFAULT_EXPORT_DESTINATION_DETENTION,
    DEFAULT_IMPORT_DESTINATION_DETENTION,
)
...
from services.fcl_freight_rate.helpers.fcl_freight_rate_bulk_operation_helpers import (
    is_price_in_range,
)
from configs.fcl_freight_rate_constants import (
...
class FclFreightRate(BaseModel):
    ...
    def set_origin_location_ids(self):
        ...
        self.origin_location_ids = [
            uuid.UUID(str(self.origin_port_id)),
            uuid.UUID(str(self.origin_country_id)),
            uuid.UUID(str(self.origin_trade_id)),
            uuid.UUID(str(self.origin_continent_id)),
        ]
    ...
    def set_destination_location_ids(self):
        ...
        self.destination_location_ids = [
            uuid.UUID(str(self.destination_port_id)),
            uuid.UUID(str(self.destination_country_id)),
            uuid.UUID(str(self.destination_trade_id)),
            uuid.UUID(str(self.destination_continent_id)),
        ]

    def validate_origin_main_port_id(self):
        if (
            self.origin_port
            and self.origin_port["is_icd"]
            and not self.rate_not_available_entry
        ):
            if (
                not self.origin_main_port_id
                or not self.origin_main_port
                or self.origin_main_port["is_icd"]
            ):
                return False
    ...
    def validate_destination_main_port_id(self):
        if (
            ...
            and self.destination_port["is_icd"]
            and not self.rate_not_available_entry
        ):
            if (
                not self.destination_main_port_id
                or not self.destination_main_port
                or self.destination_main_port["is_icd"]
            ):
                return False
    ...
    def validate_commodity(self):
        if (
            self.container_type
            and self.commodity
            in FREIGHT_CONTAINER_COMMODITY_MAPPINGS[f"{self.container_type}"]
        ):
            return True
        return False
    ...
    def set_omp_dmp_sl_sp(self):
        self.omp_dmp_sl_sp = ":".join(
            [
                str(self.origin_main_port_id or ""),
                str(self.destination_main_port_id or ""),
                str(self.shipping_line_id),
                str(self.service_provider_id),
            ]
        )
    ...
    def update_special_attributes(self):
        ...
        if self.importer_exporter_id:
            free_days_query = free_days_query.where(
                FclFreightRateFreeDay.importer_exporter_id == self.importer_exporter_id
            )
        else:
            free_days_query = free_days_query.where(
                FclFreightRateFreeDay.importer_exporter_id == None
            )
    ...
    def update_origin_local_line_item_messages(self):
        ...
        if self.origin_local:
            ...
            response = self.origin_local_data_instance.get_line_item_messages(
                self.origin_port,
                self.origin_main_port,
                self.shipping_line,
                self.container_size,
                self.container_type,
                self.commodity,
                "export",
                self.possible_origin_local_charge_codes(),
            )

        self.origin_local_line_items_error_messages = response.get(
            "line_items_error_messages"
        )
        self.is_origin_local_line_items_error_messages_present = response.get(
            "is_line_items_error_messages_present"
        )
        self.origin_local_line_items_info_messages = response.get(
            "line_items_info_messages"
        )
        self.is_origin_local_line_items_info_messages_present = response.get(
    ...
    def update_destination_local_line_item_messages(self):
        ...
        if self.destination_local:
            self.destination_local_data_instance = FclFreightRateLocalData(
                self.destination_local
            )
            response = self.destination_local_data_instance.get_line_item_messages(
                self.destination_port,
                self.destination_main_port,
                self.shipping_line,
                self.container_size,
                self.container_type,
                self.commodity,
                "import",
                self.possible_destination_local_charge_codes(),
            )

        self.destination_local_line_items_error_messages = response.get(
            "line_items_error_messages"
        )
        self.is_destination_local_line_items_error_messages_present = response.get(
            "is_line_items_error_messages_present"
        )
        self.destination_local_line_items_info_messages = response.get(
            "line_items_info_messages"
        )
        self.is_destination_local_line_items_info_messages_present = response.get(
            "is_line_items_info_messages_present"
        )
    ...
    def update_origin_free_days_special_attributes(self, free_days):
        ...
        for free_day_obj in free_days:
            if (
                free_day_obj["trade_type"] == "export"
                and free_day_obj["free_days_type"] == "detention"
            ):
                origin_detention = free_day_obj
            if (
                free_day_obj["trade_type"] == "export"
                and free_day_obj["free_days_type"] == "demurrage"
            ):
                origin_demurrage = free_day_obj
            if (
                free_day_obj["trade_type"] == "export"
                and free_day_obj["free_days_type"] == "plugin"
            ):
    ...
    def update_destination_free_days_special_attributes(self, free_days):
        ...
        for free_day_obj in free_days:
            if (
                free_day_obj["trade_type"] == "import"
                and free_day_obj["free_days_type"] == "detention"
            ):
        ...
        if (
            not "slabs" in destination_detention
            or len(destination_detention["slabs"] or []) == 0
        ):
            self.is_destination_detention_slabs_missing = True
        if (
            not "slabs" in destination_demurrage
            or len(destination_demurrage["slabs"] or []) == 0
        ):
            self.is_destination_demurrage_slabs_missing = True
        if (
            not "slabs" in destination_plugin
            or len(destination_plugin["slabs"] or []) == 0
        ):
            self.is_destination_plugin_slabs_missing = True

    def update_weight_limit_special_attributes(self):
        if self.weight_limit:
            self.is_weight_limit_slabs_missing = (
                len(self.weight_limit.get("slabs", [])) == 0
            )
        else:
    ...
    def validate_origin_local(self):
        if "origin_local" in self.dirty_fields and self.origin_local:
            if not self.origin_local_instance.validate_duplicate_charge_codes():
                raise HTTPException(
                    status_code=400, detail="Duplicate line items in Origin Local"
                )
            invalid_charge_codes = (
                self.origin_local_instance.validate_invalid_charge_codes(
                    self.possible_origin_local_charge_codes()
                )
            )
            if invalid_charge_codes:
                raise HTTPException(
                    status_code=400,
                    detail=f"{invalid_charge_codes} are invalid Origin Local line items",
                )
    ...
    def validate_destination_local(self):
        if "destination_local" in self.dirty_fields and self.destination_local:
            if not self.destination_local_instance.validate_duplicate_charge_codes():
                raise HTTPException(
                    status_code=400, detail="Duplicate line items in Destination Local"
                )
            invalid_charge_codes = (
                self.destination_local_instance.validate_invalid_charge_codes(
                    self.possible_destination_local_charge_codes()
                )
            )
            if invalid_charge_codes:
                raise HTTPException(
                    status_code=400,
                    detail=f"{invalid_charge_codes} are invalid Destination Local line items",
                )
    ...
    def validate_validity_object(self, validity_start, validity_end):
        ...
        if validity_end.date() > (
            datetime.datetime.now().date() + datetime.timedelta(days=60)
        ):
            raise HTTPException(
                status_code=400,
                detail="validity_end can not be greater than 60 days from current date",
            )
        ...
        if validity_end.date() < (
            ...
            raise HTTPException(
                status_code=400,
                detail="validity_end can not be less than 2 days from current date",
            )

        if validity_start.date() < (
            datetime.datetime.now().date() - datetime.timedelta(days=15)
        ):
            raise HTTPException(
                status_code=400,
                detail="validity_start can not be less than 15 days from current date",
            )
    ...
    def validate_line_items(self, line_items):
        ...
        if len(set(codes)) != len(codes) and (self.rate_type != "cogo_assured"):
            raise HTTPException(
                status_code=400, detail="line_items contains duplicates"
            )
        ...
        invalid_line_items = [
            code
            for code in codes
            if code.strip() not in fcl_freight_charges_dict.keys()
        ]

        if invalid_line_items:
            raise HTTPException(
                status_code=400,
                detail="line_items {} are invalid".format(
                    ", ".join(invalid_line_items)
                ),
            )
        ...
        if len([code for code in mandatory_codes if code not in codes]) > 0:
            raise HTTPException(
                status_code=400,
                detail="line_items does not contain all mandatory_codes {}".format(
                    ", ".join([code for code in mandatory_codes if code not in codes])
                ),
            )

    def get_platform_price(
        self, validity_start, validity_end, price, currency, rate_type
    ):
        freight_rates = (
            FclFreightRate.select(FclFreightRate.validities, FclFreightRate.id)
            .where(
        ...
        if freight_rates:
            for freight_rate in freight_rates:
                ...
                for t in freight_rate.validities:
                    if (validity_start <= t["validity_end"]) & (
                        validity_end >= t["validity_start"]
                    ):
                        validities.append(t)
                ...
                price = []
                for t in validities:
                    new_price = common.get_money_exchange_for_fcl(
                        {
                            "price": t["price"],
                ...
                freight_rate_min_price = min(price) if price else None

                if (
                    freight_rate_min_price is not None
                    and freight_rate_min_price < result
                ):
    ...
    def set_platform_prices(self, rate_type=DEFAULT_RATE_TYPE):
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [
                executor.submit(
                    self.get_platform_price,
                    validity_object["validity_start"],
                    validity_object["validity_end"],
                    validity_object["price"],
                    validity_object["currency"],
                    rate_type,
                )
    ...
    def set_validities_for_cogo_assured_rates(self, validities):
        ...
        new_validities = [
            ...
            if datetime.datetime.strptime(
                str(validity.validity_end).replace("T", " ").split()[0], "%Y-%m-%d"
            ).date()
            >= datetime.datetime.now().date()
        ]
        new_validities = sorted(
            new_validities,
            key=lambda validity: datetime.datetime.strptime(
                str(validity.validity_start).replace("T", " ").split()[0], "%Y-%m-%d"
            ).date(),
        )

        main_validities = []
        for new_validity in new_validities:
            new_validity.line_items = [
                dict(line_item) for line_item in new_validity.line_items
            ]
            new_validity.validity_start = (
                datetime.datetime.strptime(
                    str(new_validity.validity_start).replace("T", " ").split()[0],
                    "%Y-%m-%d",
                )
                .date()
                .isoformat()
            )
            new_validity.validity_end = (
                datetime.datetime.strptime(
                    str(new_validity.validity_end).replace("T", " ").split()[0],
                    "%Y-%m-%d",
                )
                .date()
                .isoformat()
    ...
    def set_validities(
        ...
        for validity_object in self.validities:
            ...
            validity_object_validity_start = datetime.datetime.strptime(
                validity_object["validity_start"], "%Y-%m-%d"
            ).date()
            validity_object_validity_end = datetime.datetime.strptime(
                validity_object["validity_end"], "%Y-%m-%d"
            ).date()

            line_item = [
                t
                for t in validity_object["line_items"]
                if t["code"] == other_params.get("comparison_charge_code")
            ]
            price_to_compare, currency = (
                (line_item[0]["price"], line_item[0]["currency"])
                if line_item
                else (None, None)
            )
            ...
            if (
                not deleted
                and validity_object["validity_start"]
                == validity_start.strftime("%Y-%m-%d")
                and validity_object["validity_end"] == validity_end.strftime("%Y-%m-%d")
                and validity_object["schedule_type"] == schedule_type
                and validity_object["payment_term"] == payment_term
            ):
                old_validity_id = validity_object["id"]

            if not is_price_in_range(
                other_params.get("rates_greater_than_price"),
                other_params.get("rates_less_than_price"),
                price_to_compare,
                other_params.get("comparision_currency"),
                currency,
            ):
            ...
            if (
                validity_object["schedule_type"] not in [None, schedule_type]
                and not deleted
            ):
                new_validities.append(FclFreightRateValidity(**validity_object))
                new_tags[id] = previous_tag
                continue
            if (
                validity_object["payment_term"] not in [None, payment_term]
                and not deleted
            ):
                new_validities.append(FclFreightRateValidity(**validity_object))
                new_tags[id] = previous_tag
            ...
            if validity_object_validity_end < validity_start:
                ...
                continue
            if (
                validity_object_validity_start >= validity_start
                and validity_object_validity_end <= validity_end
            ):
                new_tags[id] = previous_tag
                continue
            if (
                ...
                validity_object["validity_end"] = validity_start - datetime.timedelta(
                    days=1
                )
            ...
            if (
                ...
                validity_object["validity_start"] = validity_end + datetime.timedelta(
                    days=1
                )
                validity_object["action"] = "update"
                new_validities.append(FclFreightRateValidity(**validity_object))
            ...
            if (
                ...
                validity_object["action"] = "update"
                new_validities.append(
                    FclFreightRateValidity(
                        **{
                            **validity_object,
                            "validity_end": validity_start - datetime.timedelta(days=1),
                        }
                    )
                )
                new_tags[id] = tag
                new_validity = {
                    **validity_object,
                    "validity_start": validity_end + datetime.timedelta(days=1),
                }
        ...
        new_validities = [
            ...
            for validity in new_validities
            if datetime.datetime.strptime(
                str(validity.validity_end).split(" ")[0], "%Y-%m-%d"
            ).date()
            >= datetime.datetime.now().date()
        ]
        new_validities = sorted(
            new_validities,
            key=lambda validity: datetime.datetime.strptime(
                str(validity.validity_start).split(" ")[0], "%Y-%m-%d"
            ).date(),
        )
        ...
        for new_validity in new_validities:
            ...
            new_validity.validity_start = (
                datetime.datetime.strptime(
                    str(new_validity.validity_start).split(" ")[0], "%Y-%m-%d"
                )
                .date()
                .isoformat()
            )
            new_validity.validity_end = (
                datetime.datetime.strptime(
                    str(new_validity.validity_end).split(" ")[0], "%Y-%m-%d"
                )
                .date()
                .isoformat()
            )
        ...
        new_tags = {
            key: value for key, value in new_tags.items() if key in validity_ids
        }

        self.tags = new_tags

    def validate_before_save(self):
        schema_weight_limit = Schema(
            {"free_limit": float, Optional("slabs"): list, Optional("remarks"): list}
        )

        if self.weight_limit:
            schema_weight_limit.validate(self.weight_limit)

            self.weight_limit["slabs"] = sorted(
                self.weight_limit["slabs"], key=lambda x: x["lower_limit"]
            )

            if (
                self.weight_limit["slabs"]
                and self.weight_limit["free_limit"] != 0
                and (
                    self.weight_limit["free_limit"]
                    >= self.weight_limit["slabs"][0]["lower_limit"]
                )
            ):
                raise HTTPException(
                    status_code=400,
            ...
            for index, weight_limit_slab in enumerate(self.weight_limit["slabs"]):
                if (
                    weight_limit_slab["upper_limit"] <= weight_limit_slab["lower_limit"]
                ) or (
                    index != 0
                    and weight_limit_slab["lower_limit"]
                    <= self.weight_limit["slabs"][index - 1]["upper_limit"]
                ):
        ...
        self.destination_local_instance = FclFreightRateLocalData(
            self.destination_local
        )
        ...
        if not self.validate_service_provider_id():
            raise HTTPException(
                status_code=400, detail="Service provider is not Valid for this service"
            )
        ...
        if not self.validate_origin_main_port_id():
            raise HTTPException(
                status_code=400, detail="origin main port id is required"
            )

        if not self.validate_destination_main_port_id():
            raise HTTPException(
                status_code=400, detail="destination main port id is required"
            )
    ...
    def local_data_get_line_item_messages(self):
        location_ids = list(
            set(
                [
                    item["location_id"]
                    for item in self.origin_local["line_items"]
    ...
    def update_local_references(self):
        ...
        if len(main_port_ids) == 2:
            local_objects_query = local_objects_query.where(
                FclFreightRateLocal.main_port_id << main_port_ids
            )
        elif len(main_port_ids) == 1:
            local_objects_query = local_objects_query.where(
                (
                    FclFreightRateLocal.main_port_id.is_null(True)
                    | FclFreightRateLocal.main_port_id << main_port_ids
                )
            )
        ...
        for local in local_objects:
            if (
                local["port_id"] == self.origin_port_id
                and (
                    not self.origin_main_port_id
                    or self.origin_main_port_id == local["main_port_id"]
                )
                and local["trade_type"] == "export"
            ):
                origin_locals.append(local)
            if (
                local["port_id"] == self.destination_port_id
                and (
                    not self.destination_main_port_id
                    or self.destination_main_port_id == local["main_port_id"]
                )
                and local["trade_type"] == "import"
            ):
                destination_locals.append(local)

        origin_local_object_id = (
            origin_locals[0]["id"] if len(origin_locals) > 0 else None
        )
        destination_local_object_id = (
            destination_locals[0]["id"] if len(destination_locals) > 0 else None
        )
    ...
    def detail(self):
        ...
        if len(local_ids):
            local_charges_query = FclFreightRateLocal.select(
                ...
                FclFreightRateLocal.id << local_ids,
                (
                    ~FclFreightRateLocal.rate_not_available_entry
                    | FclFreightRateLocal.rate_not_available_entry.is_null(True)
                ),
            )
        ...
        if len(free_day_ids):
            free_days_query = FclFreightRateFreeDay.select(
                ...
                FclFreightRateFreeDay.id << free_day_ids,
                (
                    ~FclFreightRateFreeDay.rate_not_available_entry
                    | FclFreightRateFreeDay.rate_not_available_entry.is_null(True)
                ),
            )
        ...
        if self.origin_local_id in local_charges and (
            not origin_local
            or not "line_items" in origin_local
            or len(origin_local["line_items"] == 0)
        ):
            origin_local = local_charges[self.origin_local_id]

        if self.origin_detention_id in free_days_charges:
            origin_local["detention"] = free_days_charges[self.origin_detention_id] | (
                {"is_slabs_missing": self.is_origin_detention_slabs_missing}
            )
        else:
            origin_local["detention"] = {
                "free_limit": DEFAULT_EXPORT_DESTINATION_DETENTION,
                "is_slabs_missing": True,
                "slabs": [],
            }

        if self.origin_demurrage_id in free_days_charges:
            origin_local["demurrage"] = free_days_charges[self.origin_demurrage_id] | (
                {"is_slabs_missing": self.is_origin_demurrage_slabs_missing}
            )
        else:
            origin_local["demurrage"] = {
                "free_limit": 0,
                "is_slabs_missing": True,
                "slabs": [],
            }
        ...
        if self.origin_plugin_id in free_days_charges:
            origin_local["plugin"] = free_days_charges[self.origin_plugin_id] | (
                {"is_slabs_missing": self.is_origin_plugin_slabs_missing}
            )
        else:
            origin_local["plugin"] = {
                "free_limit": 0,
        ...
        if self.destination_local_id in local_charges and (
            not destination_local
            or not "line_items" in destination_local
            or len(destination_local["line_items"] == 0)
        ):
            destination_local = local_charges[self.destination_local_id]

        if self.destination_detention_id in free_days_charges:
            destination_local["detention"] = free_days_charges[
                self.destination_detention_id
            ] | ({"is_slabs_missing": self.is_destination_detention_slabs_missing})
        else:
            destination_local["detention"] = {
                "free_limit": DEFAULT_IMPORT_DESTINATION_DETENTION,
                "is_slabs_missing": True,
                "slabs": [],
            }

        if self.destination_demurrage_id in free_days_charges:
            destination_local["demurrage"] = free_days_charges[
                self.destination_demurrage_id
            ] | ({"is_slabs_missing": self.is_destination_demurrage_slabs_missing})
        else:
            destination_local["demurrage"] = {
                "free_limit": 0,
                "is_slabs_missing": True,
                "slabs": [],
            }
        ...
        if self.destination_plugin_id in free_days_charges:
            destination_local["plugin"] = free_days_charges[
                self.destination_plugin_id
            ] | ({"is_slabs_missing": self.is_destination_plugin_slabs_missing})
        else:
            destination_local["plugin"] = {
                "free_limit": 0,
                "is_slabs_missing": True,
                "slabs": [],
            }
    ...
    def create_trade_requirement_rate_mapping(self, procured_by_id, performed_by_id):
        ...
        data = {
            ...
            "last_rate_available_date": datetime.datetime.strptime(
                str(self.last_rate_available_date), "%Y-%m-%d"
            )
            .date()
            .isoformat(),
            ...
            "is_origin_local_missing": (
                not self.origin_local
                or not "line_items" in self.origin_local
                or len(self.origin_local["line_items"]) == 0
            ),
            "is_destination_local_missing": (
                not self.destination_local
                or not "line_items" in self.destination_local
                or len(self.destination_local["line_items"]) == 0
            ),
            "rate_params": {
    ...
    def get_price_for_trade_requirement(self):
        ...
        result = common.get_money_exchange_for_fcl(
            {
    ...
    def create_fcl_freight_free_days(
        self, new_free_days, performed_by_id, sourced_by_id, procured_by_id
    ):
        ...
        origin_detention_id = (
            origin_demurrage_id
        ) = destination_detention_id = destination_demurrage_id = None
        ...
        if (
            "destination_detention" in new_free_days
            and new_free_days["destination_detention"]
        ):
        ...
        if (
            "destination_demurrage" in new_free_days
            and new_free_days["destination_demurrage"]
        ):
            obj["location_id"] = self.destination_port_id
            obj["free_days_type"] = "demurrage"
            obj["trade_type"] = "import"
