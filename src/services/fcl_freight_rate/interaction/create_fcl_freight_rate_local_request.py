from services.fcl_freight_rate.models.fcl_freight_rate_local_request import (
    FclFreightRateLocalRequest,
)
...
def execute_transaction_code(request):
    ...
    if request.get("preferred_shipping_line_ids"):
        request["preferred_shipping_line_ids"] = [
            uuid.UUID(str_id) for str_id in request["preferred_shipping_line_ids"]
        ]
    ...
    local_request = (
        FclFreightRateLocalRequest.select()
        .where(
            FclFreightRateLocalRequest.source == request.get("source"),
            FclFreightRateLocalRequest.source_id == request.get("source_id"),
            FclFreightRateLocalRequest.performed_by_id
            == request.get("performed_by_id"),
            FclFreightRateLocalRequest.performed_by_type
            == request.get("performed_by_type"),
            FclFreightRateLocalRequest.performed_by_org_id
    ...
    send_notifications_to_supply_agents_local_request.apply_async(
        kwargs={"object": local_request}, queue="communication"
    )

    return {"id": local_request.id}
...
def create_audit(request, local_request_id):
    ...
    if request.get("preferred_shipping_line_ids"):
        request["preferred_shipping_line_ids"] = [
            str(str_id) for str_id in request["preferred_shipping_line_ids"]
        ]
