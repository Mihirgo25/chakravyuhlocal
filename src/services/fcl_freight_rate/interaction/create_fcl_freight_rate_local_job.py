from services.fcl_freight_rate.models.fcl_freight_rate_local_job import (
...
def execute_transaction_code(request, source):
    from services.fcl_freight_rate.fcl_celery_worker import (
        update_live_booking_visiblity_for_fcl_freight_rate_local_job_delay,
    )

    request = jsonable_encoder(request)
    params = {
        "port_id": request.get("port_id"),
        "main_port_id": request.get("main_port_id") or None,
    ...
    if not fcl_freight_rate_local_job:
        ...
        user_id = allocate_job(
            source,
            params["service_provider_id"],
            "fcl_freight_local",
            request.get("trade_type"),
        )
        ...
        if source == "live_booking":
            update_live_booking_visiblity_for_fcl_freight_rate_local_job_delay.apply_async(
                args=[fcl_freight_rate_local_job.id], countdown=1800, queue="critical"
            )
...
def update_live_booking_visiblity_for_fcl_freight_rate_local_job(job_id):
    FclFreightRateLocalJob.update(is_visible=True).where(
        (FclFreightRateLocalJob.id == job_id)
