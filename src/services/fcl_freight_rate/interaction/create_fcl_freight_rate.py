...
def add_rate_properties(request, freight_id):
    ...
    rp = (
        FclFreightRateProperties.select(FclFreightRateProperties.id)
        .where(FclFreightRateProperties.rate_id == str(freight_id))
        .first()
    )
    if not rp:
...
def create_audit(request, freight_id):
    ...
    audit_data["fcl_freight_rate_request_id"] = request.get(
        "fcl_freight_rate_request_id"
    )
    audit_data["validities"] = (
        jsonable_encoder(request.get("validities") or {})
        if rate_type == "cogo_assured"
        else None
    )
...
def create_fcl_freight_rate(request):
    ...
    from services.fcl_freight_rate.fcl_celery_worker import (
        update_fcl_freight_rate_job_on_rate_addition_delay,
    ...
    port_to_region_id_mapping = freight.set_locations()
    ...
    new_free_days["origin_detention"] = request.get("origin_local", {}).get(
        "detention", {}
    )
    new_free_days["origin_demurrage"] = request.get("origin_local", {}).get(
        "demurrage", {}
    )
    new_free_days["destination_detention"] = request.get("destination_local", {}).get(
        "detention", {}
    )
    new_free_days["destination_demurrage"] = request.get("destination_local", {}).get(
        "demurrage", {}
    )

    if request.get("origin_local") and "line_items" in request["origin_local"]:
        freight.origin_local = {
            "line_items": get_normalized_line_items(
                request["origin_local"]["line_items"]
            )
        }
    else:
    ...
    if (
        request.get("destination_local")
        and "line_items" in request["destination_local"]
    ):
        freight.destination_local = {
            "line_items": get_normalized_line_items(
                request["destination_local"]["line_items"]
            )
        }
    else:
    ...
    source = request.get("source")
    line_items = (
        get_flash_booking_rate_line_items(request)
        if source == "flash_booking"
        else request.get("line_items")
    )

    line_items = get_normalized_line_items(line_items)

    if "rate_sheet_validation" not in request and row["rate_type"] != "cogo_assured":
        freight.validate_validity_object(
            request["validity_start"], request["validity_end"]
        )
    ...
    freight.create_fcl_freight_free_days(
        new_free_days,
        request["performed_by_id"],
        request["sourced_by_id"],
        request["procured_by_id"],
    )
    ...
    adjust_dynamic_pricing(
        request, row, freight, current_validities, is_rate_extended_via_bo
    )

    if request.get("fcl_freight_rate_request_id"):
        update_fcl_freight_rate_request_in_delay.apply_async(
            kwargs={
                "request": {
                    "fcl_freight_rate_request_id": request.get(
                        "fcl_freight_rate_request_id"
                    ),
                    "reverted_rates": [
                        {
    ...
    if request.get("fcl_freight_rate_feedback_id"):
        update_fcl_freight_rate_feedback_in_delay.apply_async(
            kwargs={
                "request": {
                    "fcl_freight_rate_feedback_id": request.get(
    ...
    if (
        row["mode"] not in ["predicted", "cluster_extension"]
        and row["rate_type"] == "market_place"
        and request.get("tag") != "trend_GRI"
    ):
        update_fcl_freight_rate_job_on_rate_addition_delay.apply_async(
            kwargs={"request": request, "id": freight.id}, queue="fcl_freight_rate"
        )
        create_sailing_schedule_port_pair_coverage_delay.apply_async(
            kwargs={"request": jsonable_encoder(model_to_dict(freight))}, queue="low"
        )
...
def adjust_dynamic_pricing(
    ...
    if (
        row["mode"] in EXTENSION_ENABLED_MODES
        and not request.get("is_extended")
        and not is_rate_extended_via_bo
        and row["rate_type"] == "market_place"
    ):
    ...
    if (
        ...
        adjust_fcl_freight_dynamic_pricing.apply_async(
            kwargs={"new_rate": rate_obj, "current_validities": current_validities},
            queue="low",
        )
...
def adjust_cogoassured_price(row, request):
    ...
    cogo_id = (
        FclFreightRate.select(FclFreightRate.id)
        .where(
    ...
    if not cogo_id:
        ...
        create_fcl_freight_rate_delay.apply_async(
            kwargs={"request": params}, queue="fcl_freight_rate"
        )
    else:
...
def get_flash_booking_rate_line_items(request):
    line_items = request.get("line_items")
    if (
        request["container_type"] in ["open_top", "flat_rack", "iso_tank"]
        or request["container_size"] == "45HC"
    ):
        line_items.append(
            {
                "code": "SPE",
...
def send_stats(action, request, freight, port_to_region_id_mapping):
    ...
    send_fcl_rate_stats_in_delay.apply_async(
