...
def get_fcl_freight_rates(filters, searched_origin_port_id, searched_destination_port_id, validity_start):
    ...
    fcl_freight_query = query.where(~FclFreightRate.rate_not_available_entry, FclFreightRate.origin_port_id != searched_origin_port_id, FclFreightRate.destination_port_id != searched_destination_port_id, FclFreightRate.last_rate_available_date >= max([validity_start, datetime.now()]), FclFreightRate.rate_type == DEFAULT_RATE_TYPE)
