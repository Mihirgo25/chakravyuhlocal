...
def apply_updated_at_greater_than_time_filter(query, filters):
  query = query.where(FclFreightRate.updated_at >= datetime.fromisoformat(filters['updated_at_greater_than_time']))
  return query


def apply_updated_at_less_than_time_filter(query,filters):
  query = query.where(FclFreightRate.updated_at <= datetime.fromisoformat(filters['updated_at_less_than_time']))
  return query
...
def apply_exclude_service_provider_id_filter(query, filters):
  ...
  query = query.where(FclFreightRate.service_provider_id.not_in(exclude_service_provider_ids))
  return query
