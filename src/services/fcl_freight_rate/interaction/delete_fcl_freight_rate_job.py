...
def delete_fcl_freight_rate_job(request):
    ...
    if request.get('fcl_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in FclFreightRateJobMapping.select(FclFreightRateJobMapping.job_id).where((FclFreightRateJobMapping.shipment_id == request['shipment_id']) & (FclFreightRateJobMapping.shipment_service_id == request['shipment_service_id']))]
...
def update_mapping(status, jobs_ids):
    update_params = {'status': status,  "updated_at": datetime.now()}
    FclFreightRateJobMapping.update(update_params).where(FclFreightRateJobMapping.job_id << jobs_ids, FclFreightRateJobMapping.status.not_in(['completed', 'aborted', 'reverted'])).execute()
