...
def delete_fcl_freight_rate_job(request):
    ...
    if request.get('fcl_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in FclFreightRateJobMapping.select(FclFreightRateJobMapping.job_id).where((FclFreightRateJobMapping.shipment_id == request['shipment_id']) & (FclFreightRateJobMapping.shipment_service_id == request['service_id']))]
