...
def delete_fcl_freight_rate_job(request):
    ...
    if request["id"]:
        fcl_freight_rate_job = (
            FclFreightRateJob.update(update_params)
            .where(
                FclFreightRateJob.id == request["id"],
        ...
        fcl_freight_rate_job = (
            FclFreightRateJob.update(update_params)
            .where(
                (FclFreightRateJob.id == FclFreightRateJobMapping.job_id)
                & (FclFreightRateJobMapping.source_id == request["source_id"])
                & ~(FclFreightRateJob.status.in_(["completed", "aborted"]))
            )
            .execute()
        )
