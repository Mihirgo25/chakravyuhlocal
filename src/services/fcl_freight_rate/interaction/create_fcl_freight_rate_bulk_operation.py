...
def create_fcl_freight_rate_bulk_operation(request):
    ...
    params = {'action_name':action_name, 'data':data, 'performed_by_id':request.get('performed_by_id'), 'service_provider_id':request.get('service_provider_id'),'sourced_by_id':request.get('sourced_by_id'),'procured_by_id':request.get('procured_by_id')}
