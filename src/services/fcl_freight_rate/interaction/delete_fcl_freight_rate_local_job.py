...
def delete_fcl_freight_rate_local_job(request):
    ...
    fcl_freight_rate_local_job = FclFreightRateLocalJob.update(update_params).where(FclFreightRateLocalJob.id == request['id'], FclFreightRateLocalJob.status.not_in(['completed', 'aborted'])).execute()
