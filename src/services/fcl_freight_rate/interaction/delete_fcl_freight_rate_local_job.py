...
def delete_fcl_freight_rate_local_job(request):
    ...
    if request.get('fcl_freight_rate_local_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in FclFreightRateLocalJobMapping.select(FclFreightRateLocalJobMapping.job_id).where((FclFreightRateLocalJobMapping.shipment_id == request['shipment_id']) & (FclFreightRateLocalJobMapping.shipment_service_id == request['service_id']))]
...
def update_mapping(status, jobs_ids):
    update_params = {'status': status,  "updated_at": datetime.now()}
    FclFreightRateLocalJobMapping.update(update_params).where(FclFreightRateLocalJobMapping.job_id << jobs_ids, FclFreightRateLocalJobMapping.status.not_in(['completed', 'aborted', 'reverted'])).execute()
