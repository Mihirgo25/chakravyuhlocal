...
from services.fcl_freight_rate.models.fcl_freight_rate_local_agent import (
    FclFreightRateLocalAgent,
)
from configs.global_constants import (
    HAZ_CLASSES,
    CONFIRMED_INVENTORY,
    PREDICTED_RATES_SERVICE_PROVIDER_IDS,
)
from configs.fcl_freight_rate_constants import (
    LOCATION_HIERARCHY,
    DEFAULT_EXPORT_DESTINATION_DETENTION,
    DEFAULT_IMPORT_DESTINATION_DETENTION,
    DEFAULT_EXPORT_DESTINATION_DEMURRAGE,
    DEFAULT_IMPORT_DESTINATION_DEMURRAGE,
    DEFAULT_LOCAL_AGENT_IDS,
    DEFAULT_SHIPPING_LINE_ID,
    DEFAULT_RATE_TYPE,
)
...
def initialize_local_query(request):
    ...
    request["rate_type"] = request.get("rate_type") or DEFAULT_RATE_TYPE
    ...
    query = FclFreightRateLocal.select(
        ...
        (FclFreightRateLocal.terminal_id == request["terminal_id"])
        | (FclFreightRateLocal.terminal_id == None),
    )
...
def build_response_object(result, request):
    response_object = {
        ...
        "source": "predicted"
        if result["service_provider_id"] in PREDICTED_RATES_SERVICE_PROVIDER_IDS
        else "spot_rates",
        "tags": [],
    }
...
def build_local_line_items(result, response_object, request):
    ...
    for line_item in result["data"].get("line_items"):
        if (line_item.get("location_id")) and (
            line_item["location_id"] not in [request["port_id"], request["country_id"]]
        ):
            continue
...
def build_local_line_item_object(line_item, request):
    ...
    is_additional_service = (
        True if "additional_service" in code_config.get("tags") else False
    )
    if is_additional_service and (
        line_item["code"] not in request["additional_services"]
    ):
        return None
    ...
    if (
        is_dpd
        and ("import" in code_config.get("trade_types"))
        and (not request["include_destination_dpd"])
    ):
        return None
    ...
    if slab_value:
        slab = [
            t
            for t in line_item["slabs"]
            if (t["lower_limit"] <= slab_value) and (t["upper_limit"] >= slab_value)
        ]
        if slab:
            slab = slab[0]
        ...
        if query_result["data"].get(free_days_type):
            if query_result["data"].get(free_days_type):
                response_object[free_days_type] = (
                    query_result["data"].get(free_days_type)
                ) | ({"unit": "per_container"})
        else:
            response_object[free_days_type] = {"unit": "per_container"}

    if not response_object["detention"].get("free_limit"):
        response_object["detention"]["free_limit"] = eval(
            "DEFAULT_{}_DESTINATION_DETENTION".format(request["trade_type"].upper())
        )

    if not response_object["demurrage"].get("free_limit"):
        response_object["demurrage"]["free_limit"] = eval(
            "DEFAULT_{}_DESTINATION_DEMURRAGE".format(request["trade_type"].upper())
        )
...
def get_local_agent_ids(request):
    results = FclFreightRateLocalAgent.select(
        FclFreightRateLocalAgent.service_provider_id,
        FclFreightRateLocalAgent.location_type,
    ).where(
        FclFreightRateLocalAgent.location_id.in_(
            [request["port_id"], request["country_id"]]
        ),
    ...
    if len(new_results) == 1:
        ...
        sorted_results = sorted(
            [i for i in new_results if i[1] is not None],
            key=lambda x: LOCATION_HIERARCHY[x[1]],
        )
