...
def generate_csv_file_url_for_fcl_local(query):
    csv_urls = []
    rate_count = 0
    final_data = []
    ...
    for rate in ServerSide(query):
        if rate_count > 5000:
            rate_count = 0
            csv_url = get_csv_url("fcl_freight_local", final_data)
            csv_urls.append(csv_url)
            final_data = []

        rate_data = model_to_dict(rate)
        required_coverage_data = get_fcl_freight_local_coverage_required_data(rate_data)
