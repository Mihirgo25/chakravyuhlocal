...
def fcl_freight_expiring_rates_scheduler():
    all_fcl_critical_ports = FclFreightLocationCluster.select(
        FclFreightLocationCluster.base_port_id
    )
    all_fcl_critical_ports = jsonable_encoder(list(all_fcl_critical_ports.dicts()))
    fcl_critical_ports_except_in_vn = [
        str(i["base_port_id"])
        for i in all_fcl_critical_ports
        if str(i["base_port_id"]) not in CRITICAL_AIRPORTS_INDIA_VIETNAM
    ]
    ...
    fcl_query = FclFreightRate.select(
        *[getattr(FclFreightRate, col) for col in required_columns]
    ).where(
        (
            (FclFreightRate.origin_port_id << CRITICAL_PORTS_INDIA_VIETNAM)
            & (FclFreightRate.destination_port_id << fcl_critical_ports_except_in_vn)
        )
        | (
