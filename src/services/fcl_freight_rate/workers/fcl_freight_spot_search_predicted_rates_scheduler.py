...
def get_current_predicted_count(requirements):
    if rd:
        try:
            cached_response = rd.hget(current_processing_key, build_init_key(requirements))
...
def fcl_freight_spot_search_predicted_rates_scheduler(is_predicted, requirements):
    if is_predicted:
