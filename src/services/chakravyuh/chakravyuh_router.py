...
from params import CreateFclEstimatedRate, CreateFclLocalEstimatedRate
...
def create_fcl_local_estimated_rates(request: CreateFclLocalEstimatedRate, resp: dict = Depends(authorize_token)):
    if resp["status_code"] != 200:
        return JSONResponse(status_code=resp["status_code"], content=resp)
    if resp["isAuthorized"]:
        request.performed_by_id = resp["setters"]["performed_by_id"]
