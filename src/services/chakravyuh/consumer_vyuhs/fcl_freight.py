from services.chakravyuh.models.fcl_freight_rate_estimation import (
    FclFreightRateEstimation,
)
...
class FclFreightVyuh:
    ...
    def get_probable_rate_transformations(self, first_rate: dict = {}):
        origin_location_ids = [
            first_rate["origin_port_id"],
            first_rate["origin_country_id"],
            first_rate["origin_trade_id"],
        ]
        destination_location_ids = [
            first_rate["destination_port_id"],
            first_rate["destination_country_id"],
            first_rate["destination_trade_id"],
        ]
        ...
        transformation_query = FclFreightRateEstimation.select(
            ...
            << destination_location_ids,
            FclFreightRateEstimation.container_size
            == self.requirements["container_size"],
            FclFreightRateEstimation.container_type
            == self.requirements["container_type"],
            (
                (FclFreightRateEstimation.commodity.is_null(True))
                | (FclFreightRateEstimation.commodity == self.requirements["commodity"])
            ),
            (
                (FclFreightRateEstimation.shipping_line_id.is_null(True))
                | (FclFreightRateEstimation.shipping_line_id << shipping_line_ids)
            ),
            FclFreightRateEstimation.status == "active",
        )
        all_transformations = jsonable_encoder(list(transformation_query.dicts()))
        transformations = []
        transformations = list(
            filter(
                lambda item: item["origin_location_type"]
    ...
    def get_probable_booking_data_tranformation(self, first_rate: dict = {}):
        ...
        transformation_query = CostBookingEstimation.select(
            ...
            (
                ...
                | (CostBookingEstimation.commodity == self.requirements["commodity"])
            ),
            (
                (CostBookingEstimation.shipping_line_id.is_null(True))
                | (CostBookingEstimation.shipping_line_id << shipping_line_ids)
            ),
    ...
    def get_most_eligible_customer_transformation(
        self, probable_customer_transformations
    ):
        return probable_customer_transformations[0]

    def apply_customer_transformation(self, rate, probable_customer_transformations):
        customer_transformation_to_apply = (
            self.get_most_eligible_customer_transformation(
                probable_customer_transformations
            )
        )
        return rate
    ...
    def get_most_eligible_rate_transformation(
        self, probable_transformations: list = []
    ):
    ...
    def get_adhoc_rate(self, price):
        if (
            self.requirements["origin_country_id"] == CHINA_COUNTRY_ID
            and self.requirements["destination_country_id"] == INDIA_COUNTRY_ID
        ):
    ...
    def get_line_item_price(self, line_item, tranformed_lineitem):
        ...
        if line_item["currency"] != currency:
            lower_limit = common.get_money_exchange_for_fcl(
                {
                    "price": lower_limit,
            ...
            upper_limit = common.get_money_exchange_for_fcl(
                {
                    "price": upper_limit,
                    "from_currency": currency,
                    "to_currency": line_item["currency"],
    ...
    def apply_rate_transformation(self, rate, probable_transformations):
        probable_transformation_to_apply = self.get_most_eligible_rate_transformation(
            probable_transformations
        )
        ...
        for validity in validities:
            ...
            for line_item in line_items:
                transformed_line_item = self.get_lineitem(
                    code=line_item["code"], items=transformation_items
                )
                if transformed_line_item:
                    adjusted_lineitem = self.get_line_item_price(
                        line_item=line_item, tranformed_lineitem=transformed_line_item
                    )
    ...
    def apply_booking_data_transformation(
        self, rate, probable_booking_data_transformations
    ):
        probable_transformation_to_apply = self.get_most_eligible_rate_transformation(
            probable_booking_data_transformations
        )
        validities = rate["validities"] or []
    ...
    def apply_transformation(
        ...
        for pt in probable_transformations:
            if (
                not pt["shipping_line_id"]
                or pt["shipping_line_id"] == rate["shipping_line_id"]
            ):
        ...
        if len(rate_specific_transformations) > 0:
            new_rate, is_shipping_line = self.apply_rate_transformation(
                rate=rate, probable_transformations=rate_specific_transformations
            )
        else:
        ...
        if not is_shipping_line and len(probable_booking_data_transformations) > 0:
            new_rate = self.apply_booking_data_transformation(
                rate=new_rate,
        ...
        if len(probable_customer_transformations) > 0:
            new_rate = self.apply_customer_transformation(
                rate=new_rate,
    ...
    def apply_dynamic_pricing(self):
        ...
        probable_booking_data_transformations = (
            self.get_probable_booking_data_tranformation(first_rate)
        )
