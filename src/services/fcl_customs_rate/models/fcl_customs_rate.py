...
from configs.fcl_customs_rate_constants import (
...
class FclCustomsRate(BaseModel):
    ...
    customs_line_items_error_messages = BinaryJSONField(
        constraints=[SQL("DEFAULT '{}'::jsonb")], null=True
    )
    customs_line_items_info_messages = BinaryJSONField(
        constraints=[SQL("DEFAULT '{}'::jsonb")], null=True
    )
    ...
    cfs_line_items_error_messages = BinaryJSONField(
        constraints=[SQL("DEFAULT '{}'::jsonb")], null=True
    )
    cfs_line_items_info_messages = BinaryJSONField(
        constraints=[SQL("DEFAULT '{}'::jsonb")], null=True
    )
        ...
        self.location_ids = list(
            filter(
                ...
                [
                    ...
                    uuid.UUID(self.country_id),
                    uuid.UUID(self.trade_id),
                    uuid.UUID(self.continent_id),
                ],
            )
        )
            ...
            total_price += common.get_money_exchange_for_fcl(
                {
                    ...
                    "from_currency": line_item.get("currency"),
    ...
    def set_location(self):
        ...
        location = maps.list_locations(
            {"filters": {"id": self.location_id}, "includes": required_params}
        )["list"]
        if location:
    ...
    def validate_cargo_handling_type(self):
        if (
            self.trade_type == "export"
            and self.cargo_handling_type
            and self.cargo_handling_type not in EXPORT_CARGO_HANDLING_TYPES
        ):
            raise HTTPException(
                status_code=400, detail="Invalid cargo_handling_type for export"
            )
        if (
            self.trade_type == "import"
            and self.cargo_handling_type
            and self.cargo_handling_type not in IMPORT_CARGO_HANDLING_TYPES
        ):
            raise HTTPException(
                status_code=400, detail="Invalid cargo_handling_type for import"
            )
    ...
    def validate_commodity(self):
        if (
            self.container_type
            and self.commodity
            in CONTAINER_TYPE_COMMODITY_MAPPINGS[f"{self.container_type}"]
        ):
            return
    ...
    def validate_location_ids(self):
        ...
        if (len(location_data) != 0) and location_data[0].get("type") in [
            "seaport",
            "country",
        ]:
            ...
            self.location_type = (
                "port"
                if location_data.get("type") == "seaport"
                else location_data.get("type")
            )
            self.location = {
                key: value
                for key, value in location_data.items()
                if key in ["id", "name", "display_name", "port_code", "type"]
            }
    ...
    def validate_duplicate_line_items(self):
        ...
        for customs_line_item in self.customs_line_items:
            unique_items.add(
                str(customs_line_item["code"]).upper()
                + str(customs_line_item.get("location_id") or "")
            )
    ...
    def validate_invalid_line_items(self):
        ...
        possible_customs_charge_codes = [
            str(key) for key in self.possible_customs_charge_codes().keys()
        ]

        invalid_customs_line_items = [
            t for t in customs_line_item_codes if t not in possible_customs_charge_codes
        ]
    ...
    def mandatory_charge_codes(self):
        mandatory_charge_codes = [
            code.upper()
            for code, config in (self.possible_customs_charge_codes() or {}).items()
            if "mandatory" in (config.get("tags") or [])
        ]
    ...
    def set_platform_price(self):
        ...
        rates = (
            FclCustomsRate.select(FclCustomsRate.customs_line_items)
            .where(
                ...
                (
                    ...
                    | (FclCustomsRate.importer_exporter_id.is_null(True))
                ),
        ...
        for rate in rates:
            selected_line_items = [
                line_item
                for line_item in rate.customs_line_items
                if line_item.get("code").upper() in self.mandatory_charge_codes()
            ]
            rate_min_price = 0.0
    ...
    def update_platform_prices_for_other_service_providers(self):
        ...
        update_fcl_customs_rate_platform_prices_delay.apply_async(
            kwargs={"request": request}, queue="low"
        )
    ...
    def possible_customs_charge_codes(self):
        ...
        for code, config in fcl_custom_charges.items():
            if (
                config.get("condition") is not None
                and eval(str(config["condition"]))
                and self.trade_type in config["trade_types"]
                and "customs_clearance" in config.get("tags")
            ):
    ...
    def update_customs_line_item_messages(self):
        ...
        for code, line_items in grouped_charge_codes.items():
            ...
            code_config = {
                ...
                if "customs_clearance" in code_config.get("tags", [])
            }
            location = self.location
            if not code_config:
            ...
            if self.trade_type not in code_config["trade_types"]:
                self.customs_line_items_error_messages[code] = [
                    f"can only be added for {', '.join(code_config['trade_types'])}"
                ]
                self.is_customs_line_items_error_messages_present = True
                continue
            ...
            if line_items["unit"] not in code_config["units"]:
                self.customs_line_items_error_messages[code] = [
                    f"can only be having units {', '.join(code_config['units'])}"
                ]
                self.is_customs_line_items_error_messages_present = True
                continue
            ...
            if (
                len(code_config.get("locations", [])) == 0
                and line_items.get("location_id")
                and len(line_items.get("location_id", [])) > 0
            ):
                self.customs_line_items_error_messages[code] = [
                    "can not be added with location"
                ]
                self.is_customs_line_items_error_messages_present = True
                continue
            ...
            if (
                ...
                self.customs_line_items_error_messages[code] = [
                    "can only be added with location"
                ]
                self.is_customs_line_items_error_messages_present = True
                continue

            if len(code_config.get("locations", [])) > 0 and (
                (locations["location_id"]["type"] != "country")
                or (
                    locations["location_id"]["country_code"].upper()
                    == code_config.get("locations", [])
                )
            ):
                self.customs_line_items_error_messages[code] = [
                    f"can only contain locations {', '.join(code_config['locations'])}"
                ]
                self.is_customs_line_items_error_messages_present = True
                continue
        ...
        for code, config in possible_customs_charge_codes.items():
            ...
            if config.get("locations"):
                ...
                required_code_specific_locations = maps.list_locations(
                    {"filters": {"id": location_codes}}
                )["list"]

                location_names = [
                    value["name"] for key, value in required_code_specific_locations
                ]
                if not location_names:
                    continue

                if not grouped_charge_codes.get(code):
                    self.customs_line_items_info_messages[code] = [
                        f"is required for serving in {', '.join(location_names)}"
                    ]
                    self.is_customs_line_items_info_messages_present = True
                else:
                    given_code_specific_location_ids = list(
                        set([t["location_id"] for t in grouped_charge_codes.get(code)])
                    )
                    remaining_code_specific_location_ids = [
                        t
                        for t in required_code_specific_locations.keys()
                        if t not in given_code_specific_location_ids
                    ]
                    if not remaining_code_specific_location_ids:
                        self.customs_line_items_info_messages[code] = [
                            f"is required for serving in {', '.join([data['name'] for id, data in required_code_specific_locations.items()])}"
                        ]
                        self.is_customs_line_items_info_messages_present = True
    ...
    def update_cfs_line_item_messages(self):
        ...
        for code, line_items in grouped_charge_codes.items():
            ...
            code_config = {
                key: value
                for key, value in code_config.items()
                if "cfs" in code_config.get("tags", [])
            }
            ...
            if self.trade_type not in code_config["trade_types"]:
                self.cfs_line_items_error_messages[code] = [
                    f"can only be added for {', '.join(code_config['trade_types'])}"
                ]
                self.is_cfs_line_items_error_messages_present = True
                continue
        ...
        for code, config in possible_cfs_charge_codes.items():
            ...
            if "additional_service" in config.get(
                ...
                if grouped_charge_codes.get(code) is None:
                    self.cfs_line_items_info_messages[code] = [
                        "can be added for more conversion"
                    ]
