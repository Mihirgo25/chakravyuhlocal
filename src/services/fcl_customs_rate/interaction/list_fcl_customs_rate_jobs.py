...
def get_data(query, filters):
    ...
    for d in data:
        mappings_query = FclCustomsRateJobMapping.select(FclCustomsRateJobMapping.shipment_service_id, FclCustomsRateJobMapping.source_serial_id, FclCustomsRateJobMapping.source_id, FclCustomsRateJobMapping.shipment_id, FclCustomsRateJobMapping.status).where(FclCustomsRateJobMapping.job_id == d['id'])
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(FclCustomsRateJobMapping.select(FclCustomsRateJobMapping.job_id).where(FclCustomsRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(FclCustomsRateJob.id << job_ids)
...
def get_reverted_count(mappings_data):
    if mappings_data.shipment_id:
        result = FclCustomsRateJobMapping.select(FclCustomsRateJobMapping.id).where(
                    (FclCustomsRateJobMapping.shipment_id == mappings_data.shipment_id) &
                    (FclCustomsRateJobMapping.status == 'reverted')
                ).count()
        return result
    return None