...
possible_direct_filters = ['id', 'feedback_type', 'performed_by_org_id', 'performed_by_id', 'status', 'closed_by_id', 'country_id', 'trade_type', 'port_id', 'trade_id', 'service_provider_id', 'serial_id', 'spot_search_serial_id','container_size','container_type','cargo_handling_type']
...
def apply_validity_start_greater_than_filter(query, filters):
    query = query.where(FclCustomsRateFeedback.created_at.cast('date') >= datetime.fromisoformat(filters['validity_start_greater_than'].split('T')[0]).date())
    return query
