...
def delete_fcl_customs_rate_job(request):
    ...
    if request.get('fcl_customs_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in FclCustomsRateJobMapping.select(FclCustomsRateJobMapping.job_id).where((FclCustomsRateJobMapping.shipment_id == request['shipment_id']) & (FclCustomsRateJobMapping.shipment_service_id == request['service_id']))]
