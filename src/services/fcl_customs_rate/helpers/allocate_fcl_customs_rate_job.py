...
def allocate_fcl_customs_rate_job(source, service_provider_id):
    ...
    active_fcl_customs_users = []
    for user in online_users:
        if user['status'] == 'active':
            active_fcl_customs_users.append(user['agent_id'])
    ...
    query = (FclCustomsRateJob.select(FclCustomsRateJob.user_id, fn.Count(FclCustomsRateJob.user_id).alias('user_id_count'))
            .where((FclCustomsRateJob.user_id << active_fcl_customs_users) &
    ...
    new_users = [user for user in active_fcl_customs_users if user not in results]
