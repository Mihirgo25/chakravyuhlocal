...
from services.air_freight_rate.constants.air_freight_rate_constants import CRITICAL_AIRPORTS_MAPPINGS
import datetime
...
def critical_port_pairs_scheduler():
    ...
    fcl_critical_ports_except_in_vn = [str(i['base_port_id']) for i in all_fcl_critical_ports if str(i['base_port_id']) not in CRITICAL_PORTS_INDIA_VIETNAM]
    ...
    for service in services:
        ...
        if service == 'air_freight':
            conditions = [((AirFreightRate.origin_airport_id == item["origin_airport_id"]) & (AirFreightRate.destination_airport_id == item["destination_airport_id"]) & (AirFreightRate.airline_id == item["airline_id"])) for item in CRITICAL_AIRPORTS_MAPPINGS]
            exp = reduce(operator.or_, conditions)
            
            air_query = AirFreightRate.select(*[getattr(AirFreightRate, col) for col in REQUIRED_COLUMNS['air_freight']]).where(exp, AirFreightRate.source != 'predicted')
