...
def insert_rates_to_rms(create_params):
    ...
    for create_param in create_params:
        ...
        create_param["line_items"][0]["price"] = (
            final_bas_price_to_rms + (5 - final_bas_price_to_rms % 10)
            if final_bas_price_to_rms % 10 <= 5
            else (final_bas_price_to_rms + (10 - final_bas_price_to_rms % 10))
        )
...
def get_fcl_freight_predicted_rate(request, serviceable_shipping_lines=[]):
    ...
    for hash in serviceable_shipping_lines:
        ...
        if (
            ...
            and request["shipping_line_id"] in hash["shipping_lines"]
        ):
            all_shipping_lines = [request["shipping_line_id"]]
        else:
            all_shipping_lines = hash["shipping_lines"]

        ports_distance = maps.get_sea_route(
            {
        ...
        if ports_distance and isinstance(ports_distance, dict):
            ...
            ports_distance = get_distance(
                port_dict.get(request["origin_port_id"]),
                port_dict.get(request["destination_port_id"]),
            )
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(all_shipping_lines)
        ) as executor:
            futures = [
                executor.submit(
                    predict_rates,
                ...
                for shipping_line_id in all_shipping_lines
            ]
    ...
    create_fcl_freight_rate_feedback_for_prediction.apply_async(
        kwargs={"result": data_for_feedback}, queue="low"
    )


def predict_rates(
    origin_port_id, destination_port_id, shipping_line_id, request, ports_distance
):
    validity_start = datetime.now().date().isoformat()
    validity_end = (datetime.now() + timedelta(days=3)).date().isoformat()

    shipping_line_dict = pickle.load(
        open(
            os.path.join(
                ROOT_DIR,
                "services",
    ...
    creation_param = {
        ...
        "origin_local": {"plugin": None, "line_items": []},
        "destination_local": {"plugin": None, "line_items": []},
        ...
        creation_param["line_items"].append(
            {
                "code": "HSC",
                "unit": "per_container",
                "price": random.choice([val for val in range(500, 801, 50)]),
                "currency": "USD",
                "remarks": [],
                "slabs": [],
            }
    ...
    if (
        creation_param["destination_country_id"]
        == "541d1232-58ce-4d64-83d6-556a42209eb7"
    ):
        creation_param["line_items"][0]["price"] = (
            creation_param["line_items"][0]["price"] + 100
        )
