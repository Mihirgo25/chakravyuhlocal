...
def get_ltl_freight_rate_job_stats(
    filters={}
):
    query = get_query()
...
def get_query():
    query = LtlFreightRateJob.select(LtlFreightRateJob.id)
    return query
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(LtlFreightRateJobMapping.select(LtlFreightRateJobMapping.job_id).where(LtlFreightRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(LtlFreightRateJob.id << job_ids)
...
def apply_end_date_filter(query, filters):
    ...
    if end_date:
        ...
        query = query.where(LtlFreightRateJob.updated_at.cast("date") <= end_date.date())
    return query


def get_statistics(filters, dynamic_statistics):
    subquery = LtlFreightRateJob.select(
        fn.UNNEST(LtlFreightRateJob.sources).alias("element")
    ).alias("elements")
    ...
    if (not(filters.get("start_date") or filters.get("end_date"))) and "completed" in filters.get("status"):
        subquery = subquery.where(
            LtlFreightRateJob.updated_at.cast("date") == datetime.now().date()
        )
    stats_query = (
        LtlFreightRateJob.select(
...
def build_daily_details(query, statistics, filters):
    ...
    query = query.select(
        LtlFreightRateJob.status, fn.COUNT(LtlFreightRateJob.id).alias("count")
    ).where(LtlFreightRateJob.status != 'pending').group_by(LtlFreightRateJob.status)
...
def apply_date_filter_and_get_pending_count(query, filters):
    ...
    pending_count = query.where(LtlFreightRateJob.status == 'pending').count()
...
def build_weekly_details(query, statistics, filters):
    ...
    weekly_stats_query = query.select(
        ...
        LtlFreightRateJob.created_at.cast("date").alias("created_at"),
    ).group_by(LtlFreightRateJob.status, LtlFreightRateJob.created_at.cast("date"))
...
def apply_extra_filters(query, filters):
    ...
    query = get_filters(applicable_filters, query, LtlFreightRateJob)
    query = apply_start_date_filter(query, filters)
    query = apply_end_date_filter(query, filters)
    return query

def get_all_backlogs(filters):
    query = LtlFreightRateJob.select()
    if filters.get('user_id'):
        query = query.where(LtlFreightRateJob.user_id == filters.get('user_id'))
