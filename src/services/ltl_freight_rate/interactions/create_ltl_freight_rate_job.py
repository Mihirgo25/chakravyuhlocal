...
def execute_transaction_code(request, source):
    ...
    init_key = f'{str(params.get("origin_location_id") or "")}:{str(params.get("destination_location_id") or "")}:{str(params.get("service_provider_id") or "")}:{str(params.get("commodity") or  "")}:{str(params.get("trip_type") or "")}:{str(params.get("transit_time") or "")}:{str(params.get("density_factor") or "")}:{str(params.get("rate_type") or "")}:{str(params.get("shipment_id") or "")}:{str(params.get("source_id") or "")}'
    ...
    if not ltl_freight_rate_job:
        ...
        user_id = allocate_job(source, params['service_provider_id'], None, 'ltl_freight')
        ...
        if source == 'live_booking':
            update_live_booking_visiblity_for_ltl_freight_rate_job_delay.apply_async(args=[ltl_freight_rate_job.id], countdown=1800,queue='critical')
    ...
    ltl_freight_rate_job.status = 'pending'
    ltl_freight_rate_job.is_visible = params['is_visible']
    ltl_freight_rate_job.save()
    if source == 'live_booking':
        update_live_booking_visiblity_for_ltl_freight_rate_job_delay.apply_async(args=[ltl_freight_rate_job.id], countdown=1800,queue='critical')
    create_audit(ltl_freight_rate_job.id, request)
