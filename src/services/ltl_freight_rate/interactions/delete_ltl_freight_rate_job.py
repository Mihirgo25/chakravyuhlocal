...
def delete_ltl_freight_rate_job(request):
    ...
    if request.get('ltl_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in LtlFreightRateJobMapping.select(LtlFreightRateJobMapping.job_id).where((LtlFreightRateJobMapping.shipment_id == request['shipment_id']) & (LtlFreightRateJobMapping.shipment_service_id == request['shipment_service_id']))]
...
def update_mapping(status, jobs_ids):
    update_params = {'status': status,  "updated_at": datetime.now()}
    LtlFreightRateJobMapping.update(update_params).where(LtlFreightRateJobMapping.job_id << jobs_ids, LtlFreightRateJobMapping.status.not_in(['completed', 'aborted', 'reverted'])).execute()
