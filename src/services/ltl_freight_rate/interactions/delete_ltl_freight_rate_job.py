...
def delete_ltl_freight_rate_job(request):
    ...
    if request.get('ltl_freight_rate_feedback_ids'):
        ...
        job_ids = [ str(job.job_id) for job in LtlFreightRateJobMapping.select(LtlFreightRateJobMapping.job_id).where((LtlFreightRateJobMapping.shipment_id == request['shipment_id']) & (LtlFreightRateJobMapping.shipment_service_id == request['service_id']))]
