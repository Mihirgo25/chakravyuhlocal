...
class SouthAmericaTrailerRateEstimator():
    ...
    def constants_cost(self,distance):
        ...
        if constants:
            ...
            constants = TrailerFreightRateEstimatorConstant.select().where(
                            (TrailerFreightRateEstimatorConstant.country_code == "US"),
                            (TrailerFreightRateEstimatorConstant.status == 'active')
                            ).order_by(TrailerFreightRateEstimatorConstant.created_at.desc()).first()
    ...
    def SouthAmerica_estimate(self, container_size, container_type, containers_count, cargo_weight_per_container, trip_type):
        ''' 
        Primary Function to estimate South America prices
        '''
        print('Estimating South America rates')
        ...
        fuel_cost = fuel_used * DEFAULT_FUEL_PRICES[COUNTRY_CURRENCY_CODE_MAPPING['US']] #use fuel charge with currency
