...
def update_ftl_freight_rate_platform_prices(request):
    ...
    freight_objects = FtlFreightRate.select().where(
    ...
    (FtlFreightRate.importer_exporter_id == request.get('importer_exporter_id')) | (FtlFreightRate.importer_exporter_id.is_null(True)),
