...
def get_ftl_freight_rate_job_stats(
    filters={}
):
    query = get_query()
...
def get_query():
    query = FtlFreightRateJob.select(FtlFreightRateJob.id)
    return query
...
def apply_source_serial_id_filter(query, filters):
    ...
    subquery = list(FtlFreightRateJobMapping.select(FtlFreightRateJobMapping.job_id).where(FtlFreightRateJobMapping.source_serial_id << filters['source_serial_id']).dicts())
    job_ids = []
    for data in subquery:
        job_ids.append(data['job_id'])
    query = query.where(FtlFreightRateJob.id << job_ids)
...
def get_statistics(filters, dynamic_statistics):
    subquery = FtlFreightRateJob.select(
        fn.UNNEST(FtlFreightRateJob.sources).alias("element")
    ).alias("elements")
    ...
    if (not(filters.get("start_date") or filters.get("end_date"))) and "completed" in filters.get("status"):
        subquery = subquery.where(
            FtlFreightRateJob.updated_at.cast("date") == datetime.now().date()
        )
    stats_query = (
        FtlFreightRateJob.select(
...
def build_daily_details(query, statistics, filters):
    ...
    ).where(FtlFreightRateJob.status != 'pending').group_by(FtlFreightRateJob.status)
...
def apply_date_filter_and_get_pending_count(query, filters):
    ...
    pending_count = query.where(FtlFreightRateJob.status == 'pending').count()
...
def build_weekly_details(query, statistics, filters):
    query = query.where(
        FtlFreightRateJob.created_at.cast("date")
        >= datetime.now().date() - timedelta(days=7)
    )

    query = apply_source_filter(query, filters)
    weekly_stats_query = query.select(
        FtlFreightRateJob.status,
        fn.COUNT(FtlFreightRateJob.id).alias("count"),
        FtlFreightRateJob.created_at.cast("date").alias("created_at"),
    ).group_by(FtlFreightRateJob.status, FtlFreightRateJob.created_at.cast("date"))
