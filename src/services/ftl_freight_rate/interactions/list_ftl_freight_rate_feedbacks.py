...
possible_direct_filters = ['id', 'feedback_type', 'performed_by_id', 'closed_by_id', 'status', 'origin_location_id', 'destination_location_id', 'origin_country_id', 'destination_country_id', 'service_provider_id', 'serial_id', 'spot_search_serial_id']
...
def apply_q_filter(query, filters):
    q = str(filters.get('q', ''))
    query = query.where(FtlFreightRateFeedback.serial_id.cast("text") ** (q + "%"))
    return query
