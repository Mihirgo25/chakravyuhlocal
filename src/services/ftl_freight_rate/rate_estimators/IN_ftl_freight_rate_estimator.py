...
class INFtlFreightRateEstimator:
    ...
    def estimate(self):
        ...
        for data in applicable_rule_set:
            ...
            if self.truck_and_commodity_data['commodity_type'] in HAZ_CLASSES or self.truck_and_commodity_data['truck_body_type'] == 'reefer':
                basic_freight_charges += 0.1*basic_freight_charges
                
        commodity_weight = self.truck_and_commodity_data['commodity_weight']
        basic_freight_charges += commodity_weight*150*2  #loading and unloading charges
        result = {}
