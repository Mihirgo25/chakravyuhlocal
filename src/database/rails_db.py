        ...
        with conn:
            with conn.cursor() as cur:
                ...
                if not isinstance(id, list):
                    ...
                    id = tuple(id)
                sql = 'select organization_users.id, users.name, users.mobile_number from organization_users join users on organization_users.user_id = users.id where users.id in %s'
                cur.execute(sql, (id,))
