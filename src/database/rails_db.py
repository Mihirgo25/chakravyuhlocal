...
def get_cost_booking_data(origin_country_id, destination_country_id, codes=None):
    ...
    with conn:
        with conn.cursor() as cur:
            ...
            '''
            # sql = '''
            # SELECT shipment_fcl_freight_services.origin_port_id, shipment_fcl_freight_services.origin_country_id, shipment_fcl_freight_services.origin_trade_id, shipment_fcl_freight_services.destination_port_id, shipment_fcl_freight_services.destination_country_id, shipment_fcl_freight_services.destination_trade_id, shipment_fcl_freight_services.container_size, shipment_fcl_freight_services.container_type, shipment_collection_parties.invoice_date, shipment_collection_parties.line_items, shipment_collection_parties.invoice_url, shipment_fcl_freight_services.service_provider_id, shipment_fcl_freight_services.shipping_line_id
            # FROM shipment_collection_parties
            # INNER JOIN shipment_fcl_freight_services ON shipment_collection_parties.shipment_id = shipment_fcl_freight_services.shipment_id
            # WHERE shipment_fcl_freight_services.origin_country_id = %s AND shipment_fcl_freight_services.destination_country_id = %s AND shipment_collection_parties.status IN %s AND shipment_collection_parties.invoice_date > date_trunc('MONTH', CURRENT_DATE - INTERVAL '3 months')::DATE limit 10
            # '''
